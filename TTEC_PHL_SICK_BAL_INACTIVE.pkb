create or replace PACKAGE BODY      ttec_phl_sick_bal_inactive
AS
-- Program Name:   "PHL Monthly Sick Balance - Inactived  Employees"
--
-- Description:
...
-- RXNEHTI-ARGANO   17/05/23    R12.2 Upgrade Remediaition
-- ----------       --------    --------------------------------------------------------------------
...
-- Declare variables
      ...
      l_title           VARCHAR2 (200)
         := 'PHL Monthly Sick Balance - InActive Employees - Pay Period End Date: ';
      l_endofmonth      DATE;
           ...
           FROM (SELECT DISTINCT 'PHL' "Business Group",
                            ...
                            FROM per_all_people_f papf,
                                 ...
                                 (SELECT   pca1.assignment_id,
                                           ...
                                           proportion
                                      --FROM hr.pay_cost_allocations_f pca1  --code commented by RXNEHTI-ARGANO,17/05/23 
                                      FROM apps.pay_cost_allocations_f pca1  --code added by RXNEHTI-ARGANO,17/05/23
                                     WHERE pca1.effective_end_date =
                                              (SELECT MAX
                                                         (pca2.effective_end_date
                                                         )
                                                 --FROM hr.pay_cost_allocations_f pca2  --code commented by RXNEHTI-ARGANO,17/05/23 
                                                 FROM apps.pay_cost_allocations_f pca2  --code added by RXNEHTI-ARGANO,17/05/23
                                  ...
                                  GROUP BY pca1.assignment_id,
                                           pca1.cost_allocation_keyflex_id,
                                           proportion) alloc,
                                 --hr.pay_cost_allocation_keyflex asg_cost,  --code commented by RXNEHTI-ARGANO,17/05/23 
                                 apps.pay_cost_allocation_keyflex asg_cost,  --code added by RXNEHTI-ARGANO,17/05/23
                                 apps.fnd_lookup_values fnd_hourly,
                                 hr_organization_units org,
                                 --hr.pay_cost_allocation_keyflex i_org,  --code commented by RXNEHTI-ARGANO,17/05/23 
                                 apps.pay_cost_allocation_keyflex i_org,  --code added by RXNEHTI-ARGANO,17/05/23
...
-------------------------------------------------------------------------------------------------------------------------
   ...
   EXCEPTION
      ...
      THEN
         ...
         ROLLBACK;
   END write_file;
END ttec_phl_sick_bal_inactive;
/
show errors;
/