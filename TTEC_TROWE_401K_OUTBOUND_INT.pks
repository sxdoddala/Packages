create or replace PACKAGE      TTEC_TROWE_401K_OUTBOUND_INT AUTHID CURRENT_USER AS
--
-- Program Name:  TTEC_TROWE_401K_OUTBOUND_INT
-- /* $Header: TTEC_TROWE_401K_OUTBOUND_INT.pks 1.0 2011/10/14  chchan ship $ */
--
-- /*== START ================================================================================================*\
...
--     10.0  04/07/2020 CChan    Exclude employee who is assigned to Payroll Name that has '%DO NOT USE%'
...
--     1.0   15/05/23   RXNETHI-ARGANO  R12.2 Upgrade Remediation 
-- \*== END =====================================
    -- Error Constants
    /*
	START R12.2 Upgrade Remediation
	code commented bu RXNETHI-ARGANO,15/05/23
	g_application_code   cust.ttec_error_handling.application_code%TYPE := 'BEN';
    g_interface          cust.ttec_error_handling.INTERFACE%TYPE        := 'BOA 401K Intf';
    g_package            cust.ttec_error_handling.program_name%TYPE     := 'TTEC_TROWE_401K_OUTBOUND_INT';
    ...
    --g_bal_type_id_401K_Roth             hr.pay_balance_types.BALANCE_TYPE_ID%TYPE :=NULL;  /* 11.0 */
    --g_bal_type_id_401K_CatchupRoth      hr.pay_balance_types.BALANCE_TYPE_ID%TYPE :=NULL;    /* 11.0 */
    --g_bal_type_id_401K_Bonus_Roth       hr.pay_balance_types.BALANCE_TYPE_ID%TYPE :=NULL; /* 11.0 */
	--g_bal_type_id_401K_Roth_ER          hr.pay_balance_types.BALANCE_TYPE_ID%TYPE :=NULL;           --12.0
	--g_bal_type_id_401K_BonusRothER      hr.pay_balance_types.BALANCE_TYPE_ID%TYPE :=NULL;           --12.0
	--g_bal_type_id_401K_CatUpRothER      hr.pay_balance_types.BALANCE_TYPE_ID%TYPE :=NULL;           --12.0
	--g_bal_type_id_401K_Bonus_ER         hr.pay_balance_types.BALANCE_TYPE_ID%TYPE :=NULL;           --12.0
	--g_bal_type_id_401K_Catchup_ER       hr.pay_balance_types.BALANCE_TYPE_ID%TYPE :=NULL;           --12.0
	--g_bal_type_id_401K_ER               hr.pay_balance_types.BALANCE_TYPE_ID%TYPE :=NULL;           --12.0
    --g_bal_type_id_Pre_Tax_401k_ER       hr.pay_balance_types.BALANCE_TYPE_ID%TYPE :=NULL;           --12.0*/
    --code added by RXNETHI-ARGANO,15/05/23
	g_application_code   apps.ttec_error_handling.application_code%TYPE := 'BEN';
    g_interface          apps.ttec_error_handling.INTERFACE%TYPE        := 'BOA 401K Intf';
    g_package            apps.ttec_error_handling.program_name%TYPE     := 'TTEC_TROWE_401K_OUTBOUND_INT';
    ...
    g_bal_type_id_401K_Roth             apps.pay_balance_types.BALANCE_TYPE_ID%TYPE :=NULL;  /* 11.0 */
    g_bal_type_id_401K_CatchupRoth      apps.pay_balance_types.BALANCE_TYPE_ID%TYPE :=NULL;    /* 11.0 */
    g_bal_type_id_401K_Bonus_Roth       apps.pay_balance_types.BALANCE_TYPE_ID%TYPE :=NULL; /* 11.0 */
	g_bal_type_id_401K_Roth_ER          apps.pay_balance_types.BALANCE_TYPE_ID%TYPE :=NULL;           --12.0
	g_bal_type_id_401K_BonusRothER      apps.pay_balance_types.BALANCE_TYPE_ID%TYPE :=NULL;           --12.0
	g_bal_type_id_401K_CatUpRothER      apps.pay_balance_types.BALANCE_TYPE_ID%TYPE :=NULL;           --12.0
	g_bal_type_id_401K_Bonus_ER         apps.pay_balance_types.BALANCE_TYPE_ID%TYPE :=NULL;           --12.0
	g_bal_type_id_401K_Catchup_ER       apps.pay_balance_types.BALANCE_TYPE_ID%TYPE :=NULL;           --12.0
	g_bal_type_id_401K_ER               apps.pay_balance_types.BALANCE_TYPE_ID%TYPE :=NULL;           --12.0
    g_bal_type_id_Pre_Tax_401k_ER       apps.pay_balance_types.BALANCE_TYPE_ID%TYPE :=NULL;           --12.0
...
--END R12.2 Upgrade Remediation
  ...
  -- declare cursors
    ...
    select ttec_library.get_directory('CUST_TOP')||'/data/EBS/HC/Benefits/TRP/Outbound' file_path       /* 9.0.1 */
       , 'XM106263' ||  decode(apps.TTEC_GET_INSTANCE,'PROD','.DAT','.DAT') file_name /* 9.0.1 */
    from V$DATABASE;
...
--             ,past.USER_STATUS
             ...
             , CASE WHEN past.USER_STATUS = 'Active Assignment'   THEN CASE WHEN (select count(*) from hr.per_periods_of_service ppos2 where ppos2.person_id = papf.person_id) = 1 THEN '03' -- '06' previously /* 9.0.2 */
                                                                        ELSE '03' -- no rehire '97' /* 9.0.2 */
                                                                       END
                    ...
                    ELSE CASE WHEN (select count(*) from hr.per_periods_of_service ppos2 where ppos2.person_id = papf.person_id) = 1 THEN '03' -- '06' previously /* 9.0.2 */
                         ELSE '03' -- no rehire '97' /* 9.0.2 */
                         END
			 ...
			 , pjob.ATTRIBUTE20 GCA_Code                   --11.0
			 , pjob.ATTRIBUTE5 Job_Familiy                 --11.0
          /*
		  STAR R12.2 Upgrade Remediation
		  ...
		  FROM hr.per_periods_of_service ppos
             ...
             , hr.pay_all_payrolls_f pay /* 10.0 
			 */
		  --code added by RXNETHI-ARGANO,15/05/23
		  FROM apps.per_periods_of_service ppos
             ...
             , apps.pay_all_payrolls_f pay /* 10.0 */
		  --END R12.2 Upgrade Remediation
			 , per_jobs pjob              --11.0
             /* 2.5 Begin */
             , (   SELECT ppos.person_id, actual_termination_date,
                     ...
                     --FROM hr.per_periods_of_service ppos --code commented by RXNETHI-ARGANO,15/05/23
					 FROM apps.per_periods_of_service ppos --code added by RXNETHI-ARGANO,15/05/23
                    WHERE ppos.date_start =
                             (SELECT MAX (ppos2.date_start)
                                --FROM hr.per_periods_of_service ppos2 --code commented by RXNETHI-ARGANO,15/05/23
								FROM apps.per_periods_of_service ppos2 --code added by RXNETHI-ARGANO,15/05/23
                              -- WHERE ppos2.date_start <= SYSDATE /* 5.5 */
                               WHERE ppos2.date_start <= p_end_date /* 5.5 */
                                 and  ppos2.date_start =  (SELECT MAX (ppos3.date_start)
                                         --FROM hr.per_periods_of_service ppos3 --code commented by RXNETHI-ARGANO,15/05/23
										 FROM apps.per_periods_of_service ppos3 --code added by RXNETHI-ARGANO,15/05/23
...
/* 7.5  BEGIN */                 and (   (     ppos.actual_termination_date is not null
         ...
         WHERE ppos.period_of_service_id(+) = paaf1.period_of_service_id
           ...
           and paaf.effective_start_date =  (SELECT MAX(paaf2.effective_start_date)
                                            --FROM hr.per_all_assignments_f paaf2 --code commented by RXNETHI-ARGANO,15/05/23
											FROM apps.per_all_assignments_f paaf2 --code added by RXNETHI-ARGANO,15/05/23
           ...
           AND ppos.date_start =(SELECT MAX(date_start)
                                           --FROM hr.per_periods_of_service pps1 --code commented by RXNETHI-ARGANO,15/05/23
										   FROM apps.per_periods_of_service pps1 --code added by RXNETHI-ARGANO,15/05/23
...
--           AND papf.employee_number = '3280287' --in ('3001789' ,'3260140','3287520')
--           AND papf.employee_number in  ('3266009','3153036','3266009','3248855','3280569','3285355','3010695' ) /* 7.5 */
--          AND papf.employee_number in  ('3240630','3191096' )
...
--                           )
           AND paaf.job_id = pjob.job_id (+)              --11.0
		   AND hla.location_id = paaf.location_id
           AND pay.PAYROLL_ID = paaf.PAYROLL_ID /* 10.0 */
           AND upper(pay.PAYROLL_NAME) not like '%DO NOT USE%' /* 10.0 */
           AND hla.attribute2 NOT IN ( SELECT lookup_code
                                         --FROM applsys.fnd_lookup_values --code commented by RXNETHI-ARGANO,15/05/23
										 FROM apps.fnd_lookup_values --code added by RXNETHI-ARGANO,15/05/23
