create or replace PACKAGE BODY      ttec_phl_sick_bal_active
AS
-- Program Name:   "PHL Monthly Sick Balance - Active  Employees"
--
-- Description:
...
-- RXNETHI-ARGANO   11/MAY/23   R12.2 Upgrade Remediation
-- ----------       --------    --------------------------------------------------------------------
...
-- Declare variables
      ...
      l_title           VARCHAR2 (200)
         := 'PHL Monthly Sick Balance - Active Employees - Pay Period End Date: ';
      l_endofmonth      DATE;
...
-- get requireed info for transmission
      ...
      IS
         ...
         SELECT    "Business Group"
                ...
                "Payrate", "Salary Basis", "Sick Hours",
                ROUND ("Payrate" * "Sick Hours" * NVL ("Proportion", 1),
                       2
                      ) "Sick Dollars",
           ...
           FROM (SELECT DISTINCT 'PHL' "Business Group",
                                 ...
                                 (SELECT pai.action_information6
                                   ...
                                   WHERE pai.assignment_id =
                                     ...
                                     AND pai.action_information_category =
                                                           'EMPLOYEE ACCRUALS'
                                     AND pai.action_information4 = 'Sick'
                                 ...
                                 (apps.ttec_get_pto_accrual_rate_phl.ttec_get_accrual_rate_sick
                                                          (paaf.assignment_id,
                                                           l_endofmonth
                                                          )
                                 ) "Rate"
                            FROM per_all_people_f papf,
                                 ...
                                 (SELECT   pca1.assignment_id,
                                           ...
                                           proportion
                                      --FROM hr.pay_cost_allocations_f pca1 --code commented by RXNETHI-ARGANO,11/05/23
									  FROM apps.pay_cost_allocations_f pca1 --code added by RXNETHI-ARGANO,11/05/23
                                     WHERE pca1.effective_end_date =
                                              (SELECT MAX
                                                         (pca2.effective_end_date
                                                         )
                                                 --FROM hr.pay_cost_allocations_f pca2 --code commented by RXNETHI-ARGANO,11/05/23
												 FROM apps.pay_cost_allocations_f pca2 --code added by RXNETHI-ARGANO,11/05/23
                                  ...
                                  GROUP BY pca1.assignment_id,
                                           pca1.cost_allocation_keyflex_id,
                                           proportion) alloc,
                                 --hr.pay_cost_allocation_keyflex asg_cost, --code commented by RXNETHI-ARGANO,11/05/23
								 apps.pay_cost_allocation_keyflex asg_cost, --code added by RXNETHI-ARGANO,11/05/23
                                 hr_organization_units org,
                                 --hr.pay_cost_allocation_keyflex i_org, --code commented by RXNETHI-ARGANO,11/05/23
								 apps.pay_cost_allocation_keyflex i_org,   --code added by RXNETHI-ARGANO,11/05/23
   ...
   BEGIN
      ...
      l_rec :=
         'Business Group|Employee Number|Employee Full Name|Hire Date|Location Code|Location|Proportion|Location Override|Client|Department|Department Override|Assignment Number|Assignment Status|Employment Category|Payrate|Salary Basis|Sick Hours|Sick Dollars|Accrual Rate per Pay Period|Job|End Of Month';
      ...
      LOOP
         l_rec :=
            ...
            || sel."Salary Basis"
            || '|'
            || TO_CHAR (sel."Sick Hours")
            || '|'
            || TO_CHAR (sel."Sick Dollars")
            || '|'
