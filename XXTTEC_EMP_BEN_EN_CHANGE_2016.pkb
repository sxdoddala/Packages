create or replace PACKAGE BODY XXTTEC_EMP_BEN_EN_CHANGE_2016
/********************************************************************************
PROGRAM NAME:   XXTTEC_EMP_BEN_EN_CHANGE_2016
...
-------------------  -------- ------------  -----------------------------------------
Jeeva A     1.0      18-Nov-2015   Initial Version
Lalitha.N   1.1      12-JAN-2016   hr_person_type_usage_info- Changed date parameter
RXNETHI-ARGANO 1.0   13-JUL-2023   R12.2 Upgrade Remediation
********************************************************************************/
AS
   PROCEDURE main (
...
272,'METLIFE46',258,'METLIFE45',254,'METLIFE50',256,'METLIFE53',260,'METLIFE52',250,'METLIFE51',252,'METLIFE49',264,
'VSP008',277,'METLIFE53',278,'METLIFE51'
,
7871,'TELE001',
275,'TELE002',
7873,'TELE003',
...
7875,'PPL010'
                ...
                (SELECT MAX (rates.rt_strt_dt)
                   --FROM ben.ben_prtt_rt_val rates  --code commented by RXNETHI-ARGANO,13/07/23
                   FROM apps.ben_prtt_rt_val rates   --code added by RXNETHI-ARGANO,13/07/23
                  WHERE rates.prtt_enrt_rslt_id =
                                         pen.prtt_enrt_rslt_id
                    AND TRUNC (p_date) BETWEEN rt_strt_dt AND rt_end_dt)
                                                              ded_start_date,
                DECODE (pen.pl_id,
                        9871 , DECODE (SUBSTR (opt.NAME, 1, 8),
                                     'Post Tax', 'N',
                                     'Y'
                                    ),
                                     10871 , DECODE (SUBSTR (opt.NAME, 1, 8),
                                     'Post Tax', 'N',
                                     'Y'
                                    ),
                                     11871 , DECODE (SUBSTR (opt.NAME, 1, 8),
                                     'Post Tax', 'N',
                                     'Y'
                                    ),
                        247, 'N',
		   ...
		   code commented by RXNETHI-ARGANO,13/07/23
		   ...
		   */
		   ...
		   --END R12.2 Upgrade Remediation
                (SELECT *
                   --FROM ben.ben_pl_f   --code commented by RXNETHI-ARGANO,13/07/23
                   FROM apps.ben_pl_f    --code added by RXNETHI-ARGANO,13/07/23
       ...
       AND pl_typ_id IN (86,22,26,32,87,85,31,24,37,30,25,23,29,27,28,2528,2529,508,4528,129)) BPL,
                (SELECT *
                   --FROM ben.ben_opt_f  --code commented by RXNETHI-ARGANO,13/07/23
                   FROM apps.ben_opt_f   --code added by RXNETHI-ARGANO,13/07/23
                  WHERE TRUNC (p_date) BETWEEN effective_start_date
                                           AND effective_end_date) opt,
                --ben.ben_oipl_f opl,   --code commented by RXNETHI-ARGANO,13/07/23
                apps.ben_oipl_f opl,    --code added by RXNETHI-ARGANO,13/07/23
                (SELECT papf_cont.national_identifier dep_social_num,
                   ...
                   FROM per_all_people_f papf_cont,
                        ...
                        --ben.ben_elig_cvrd_dpnt_f dep,  --code commented by RXNETHI-ARGANO,13/07/23
                        apps.ben_elig_cvrd_dpnt_f dep,   --code added by RXNETHI-ARGANO,13/07/23
          ...
          WHERE
            ...
            AND hr_person_type_usage_info.get_user_person_type
                                             (TRUNC (p_date),
                                              pap.person_id
                                             ) = 'PEO Employee' --V1.1 changed to p_date
...
7875,'PPL010'
   ...
   BEGIN
      ...
      XXTTEC_EMP_BEN_EN_CHANGE_2016.write_process
                                       (   'TELETECH_PEO_EMP_BENEFIT_CHANGE_2016_'
                                        || v_dt_time
                                        || '.txt',
                                        '',
                                        'W',
      ...
      /*
	  START R12.2 Upgrade Remediaition
	  code commented by RXNETHI-ARGANO,13/07/23
	  BEGIN
         v_string :=
...
AND '
            ...
            || '  DECODE(pen.pl_id,9871,DECODE(SUBSTR(opt.name,1,8),'
            || sq
            || 'Post Tax'
            || sq
            || ','
...
ORDER BY
pap.employee_number';
--END R12.2 Upgarade Remediaition
   ...
   END WRITE_PROCESS;
END XXTTEC_EMP_BEN_EN_CHANGE_2016;
/
show errors;
/