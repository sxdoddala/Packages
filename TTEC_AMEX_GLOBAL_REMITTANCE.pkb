create or replace PACKAGE BODY      TTEC_AMEX_GLOBAL_REMITTANCE AS
--
-- Program Name:  TTEC_AMEX_GLOBAL_REMITTANCE_EXTRACT
--
...
-- -------  ----------       --------    --------------------------------------------------------------------
-- 1.0      C. Chan			3/6/2016     Initial Rollout
-- 1.1      C. Chan			8/10/2016    Add additional restriction to prevent P-card being selected + Stop process if no payment was made
-- 1.2      C. Chan			8/19/2016    Change the condition on inactive_date to allow pickup payment based on invoice date
...
PROCEDURE gen_global_amex_pay_file(errcode varchar2, errbuff varchar2, p_process_date        IN VARCHAR2
                                                                     , p_Global_Corp_Id      IN VARCHAR2
																  ) IS
...
--        || rpad(p_Global_Corp_Id,15,' ')                                  -- Field #2 Global Corporate Identifier
...
cursor c_tot_record_process is
	   ...
	   --START R12.2 Upgrade Remediation
         /*FROM     ap.ap_invoices_all ai				 -- Commented code by IXPRAVEEN-ARGANO, 02/05/2023 
                , ap.ap_invoice_payments_all aip
                , ap.ap_checks_all ac
                , ap.ap_expense_report_headers_all aeh
                , ap.ap_card_programs_all acp
                , ap.ap_cards_all aca,*/
			FROM     APPS.ap_invoices_all ai			--  code Added by IXPRAVEEN-ARGANO, 02/05/2023
     ...
     --    AND ai.invoice_num = 'TTUS7983327.1'
         AND      ac.check_id  --=  '1100061787' --'1100061504' --p_check_num   -- p_check_number
            in (
                select aip1.CHECK_ID
                --START R12.2 Upgrade Remediation
				/*from ap.ap_invoice_payments_all aip1		-- Commented code by IXPRAVEEN-ARGANO, 02/05/2023
                   , ap.ap_checks_all ac1 */
				   from APPS.ap_invoice_payments_all aip1	--  code Added by IXPRAVEEN-ARGANO, 02/05/2023
         ...
         --AND      aca.inactive_date IS NULL; /* 1.2 */
...
--     hr.HR_LOCATIONS_ALL l		-- Commented code by IXPRAVEEN-ARGANO, 02/05/2023
--     APPS.HR_LOCATIONS_ALL 		--  code Added by IXPRAVEEN-ARGANO, 02/05/2023
...
DECODE(c.currency_code,'EUR',  (select l.COUNTRY
                                from apps.HR_ORGANIZATION_UNITS o,
                                     --hr.HR_LOCATIONS_ALL l		-- Commented code by IXPRAVEEN-ARGANO, 02/05/2023
									 APPS.HR_LOCATIONS_ALL l			--  code Added by IXPRAVEEN-ARGANO, 02/05/2023
                                where o.location_id is not null
                                and o.location_id = l.location_id
...
FROM apps.hr_operating_units hou
   --, gl.gl_ledgers gl				-- Commented code by IXPRAVEEN-ARGANO, 02/05/2023
   , APPS.gl_ledgers gl               --  code Added by IXPRAVEEN-ARGANO, 02/05/2023
   , gl_currencies c
...
and rownum < 2
		...
		--START R12.2 Upgrade Remediation
         /*FROM     ap.ap_invoices_all ai
                , ap.ap_invoice_payments_all aip
                , ap.ap_checks_all ac
                , ap.ap_expense_report_headers_all aeh		-- Commented code by IXPRAVEEN-ARGANO, 02-May-2023
                , ap.ap_card_programs_all acp
                , ap.ap_cards_all aca,
				*/
		FROM     APPS.ap_invoices_all ai
                , APPS.ap_invoice_payments_all aip
                , APPS.ap_checks_all ac
                , APPS.ap_expense_report_headers_all aeh		--  code Added by IXPRAVEEN-ARGANO, 02-May-2023
                , APPS.ap_card_programs_all acp
                , APPS.ap_cards_all aca,	
					--END R12.2.10 Upgrade remediation
                  apps.iby_creditcard  ic
     ...
     --    AND ai.invoice_num = 'TTUS7983327.1'
         AND      ac.check_id  --=  '1100061787' --'1100061504' --p_check_num   -- p_check_number
            in (
                select aip1.CHECK_ID
				--START R12.2 Upgrade Remediation
                /*from ap.ap_invoice_payments_all aip1			-- Commented code by IXPRAVEEN-ARGANO, 02-May-2023
                 --  , ap.ap_checks_all ac1	*/
				 
				from APPS.ap_invoice_payments_all aip1			-- code Added by IXPRAVEEN-ARGANO, 02-May-2023
                   , APPS.ap_checks_all ac1						
				   --END R12.2.10 Upgrade remediation
                where aip1.REMIT_TO_SUPPLIER_NAME like '%AMERICAN%EXPRESS%'
                  and aip1.CHECK_ID = ac1.CHECK_ID
...
--from CUST.TT_GLOBAL_AMEX_REMITTANCE_LOG l2		-- Commented code by IXPRAVEEN-ARGANO, 02-May-2023
from APPS.TT_GLOBAL_AMEX_REMITTANCE_LOG l2			--  code Added by IXPRAVEEN-ARGANO, 02-May-2023
where l2.amex_Payment_Posting_Date = l_process_date
...
/* 02 - 60 */        , ' '  Filler -- Filler
--FROM CUST.TT_GLOBAL_AMEX_REMITTANCE_LOG l			-- Commented code by IXPRAVEEN-ARGANO, 02-May-2023
FROM APPS.TT_GLOBAL_AMEX_REMITTANCE_LOG l			--  code Added by IXPRAVEEN-ARGANO, 02-May-2023
...
BEGIN
    ...
    ELSE
       l_process_date := to_date(p_process_date);
    END IF;
...
--    close c_directory_path;
   ...
   FND_FILE.PUT_LINE(FND_FILE.Log,'TTEC_AMEX_GLOBAL_REMITTANCE_EXTRACT');
   FND_FILE.PUT_LINE(FND_FILE.Log,'Submitted On: ' ||to_char(SYSDATE,'DD-MON-RRRR HH24:MI:SS'));
   FND_FILE.PUT_LINE(FND_FILE.Log,'');
   FND_FILE.PUT_LINE(FND_FILE.Log,'=============================================================================');
   FND_FILE.PUT_LINE(FND_FILE.Log,'                          Parameters');
   FND_FILE.PUT_LINE(FND_FILE.Log,'=============================================================================');
   FND_FILE.PUT_LINE(FND_FILE.Log,'            Payment date: '||l_process_date);
   FND_FILE.PUT_LINE(FND_FILE.Log,'  Global Corp Identifier: '||p_Global_Corp_Id);
   FND_FILE.PUT_LINE(FND_FILE.Log,'           GRU File Name: '||p_FileName);
    ...
    For v_rec in c_detail_record loop
        ...
        l_rec :=
           ...
           -- || nvl(substr(lpad(v_rec.Company_ID ,19,'0'),1,19),lpad('0',19,'0'))               -- Field #7  The control account associated with the account number. Sourced from GL1205 and GL1025 Files.
            ...
            || rpad(' ',178,' ');/* 1.3 restore original */
            --|| rpad(' ',177,' '); /* 1.3 restore original */


        l_tot_rec_count := l_tot_rec_count + 1;
        utl_file.put_line(v_remittance_file, l_rec);

        --INSERT INTO CUST.TT_GLOBAL_AMEX_REMITTANCE_LOG			-- Commented code by IXPRAVEEN-ARGANO, 02/05/2023
        INSERT INTO APPS.TT_GLOBAL_AMEX_REMITTANCE_LOG              --  code Added by IXPRAVEEN-ARGANO, 02/05/2023
        (
        AMEX_RECORD_TYPE	,
        AMEX_REMIT_AMT_SIGN	,
    ...
    For v_rec in c_summary_record loop
        ...
        l_rec :=
                ...
                || rpad(' ',267,' ');    /* 1.3 restore original */                              -- Field #18 Filler -- Filler
                --|| rpad(' ',266,' ');    /* 1.3 restore original */                                -- Field #18 Filler -- Filler
    ...
    l_rec := '09'                                                                         -- Field #1 Indicates whether the record is Header, Detail, Market Summary or Trailer
        ...
        ||  rpad(' ',383,' ');    /* 1.3 restore original */                              -- Field #3 Space Fill
       -- ||  rpad(' ',382,' ');   /* 1.3 restore original */                             -- Field #3 Space Fill
 ...
 -- DELETE FROM CUST.TT_GLOBAL_AMEX_REMITTANCE_LOG		-- Commented code by IXPRAVEEN-ARGANO, 02-May-2023
 DELETE FROM APPS.TT_GLOBAL_AMEX_REMITTANCE_LOG			--  code Added by IXPRAVEEN-ARGANO, 02-May-2023
...
EXCEPTION
    WHEN NO_REMITTANCE THEN
        apps.Fnd_File.put_line (apps.Fnd_File.log,'No GLOBAL AMEX CARD payment made to AMERICAN EXPRESS for '||l_process_date||' aborting the process...' );
    ...
    WHEN OTHERS THEN
        ...
        RAISE_APPLICATION_ERROR(-20003,'Exception OTHERS in gen_global_amex_pay_file: '||l_msg);
		ROLLBACK;

END gen_global_amex_pay_file;

END TTEC_AMEX_GLOBAL_REMITTANCE;
/
show errors;
/