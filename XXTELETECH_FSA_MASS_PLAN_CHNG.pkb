create or replace PACKAGE BODY XXTELETECH_FSA_MASS_PLAN_CHNG
 /********************************************************************************
  PROGRAM NAME:   XXTELETECH_FSA_MASS_PLAN_CHNG
  ...
  DATE:           10-NOV-2013
  CALLING FROM   :  Teletech PEO Employee FSA and Mass Transit Changes Program
  ----------------
  MODIFICATION LOG
  ----------------
...
CURSOR plan_term_det(p_date DATE,
               p_start_date DATE,
               p_end_date DATE)
IS
(
(SELECT   --Terminated Plan Details Fetch
       ...
       pap.full_name full_name,
       bpl.name VIN_PLAN,
       DECODE(pen.pl_id,331,(SELECT MIN(rt_val) FROM BEN_PRTT_RT_VAL
                             WHERE prtt_enrt_rslt_id = pen.prtt_enrt_rslt_id
                             AND ((TRUNC(p_date) BETWEEN RT_STRT_DT AND RT_END_DT) OR ( TRUNC(RT_END_DT) BETWEEN p_start_date AND p_end_date))
...
--END R12.2.11 Upgrade remediation	 
       ...
       WHERE pen.pl_id = bpl.pl_id
       AND pen.person_id = pap.person_id
       AND ((TRUNC(p_date) BETWEEN pen.effective_start_date AND pen.effective_end_date) OR (TRUNC(pen.effective_end_date) BETWEEN p_start_date AND p_end_date))
       AND TRUNC(pen.enrt_cvg_thru_dt) BETWEEN p_start_date AND p_end_date
       AND  pen.prtt_enrt_rslt_stat_cd IS NULL
       AND trunc(p_date) BETWEEN pap.effective_start_date AND pap.effective_END_date
       AND hr_person_type_usage_info.get_user_person_type ( trunc(pap.effective_start_date), pap.person_id ) = 'PEO Employee'
       AND pap.current_employee_flag = 'Y'
...
--END R12.2.11 Upgrade remediation
       ...
       AND TRUNC(pen.enrt_cvg_thru_dt) BETWEEN p_start_date AND p_end_date
       AND  pen.prtt_enrt_rslt_stat_cd IS NULL
       AND TRUNC(pap.effective_END_date) BETWEEN p_start_date AND p_end_date
       AND hr_person_type_usage_info.get_user_person_type ( trunc(pap.effective_start_date), pap.person_id ) = 'PEO Employee'
       AND pap.business_group_id = 325
)
MINUS
...
AND TIME_PERIOD_ID = (SELECT MIN(TIME_PERIOD_ID) FROM PER_TIME_PERIODS
                      WHERE TRUNC(v_date) BETWEEN START_DATE AND default_dd_date
...
fnd_file.put_line(fnd_file.output,v_columns_str);
  ...
  FOR cur_term_det IN plan_term_det( v_date,
                                     v_start_date,
                                     v_end_date)

 LOOP

v_columns_str := cur_term_det.employee_number||'	'||cur_term_det.full_name||'	'||cur_term_det.vin_plan||'	'||TO_CHAR(cur_term_det.cvg_amt,'999999999.99')||'	'||TO_CHAR(cur_term_det.start_date,'MM/DD/YYYY');
...
END XXTELETECH_FSA_MASS_PLAN_CHNG;
/
show errors;
/