create or replace PACKAGE BODY ttec_populate_apbank_hr_au AS
  --************************************************************************************--
  --*                                                                                  *--
  --*     Program Name: ttec_populate_apbank_hr_au                                     *--

  --*                                                                                  *--
  --*     Description:  Update AP Vendor Banking information from HR
  --*                   employee bank info.
  --*                   Update those vendors that are employees only
  --*                   Copied from   ttec_populate_apbank_hr                          *--
  --*                                                                                  *--
  ...
  --*---------             ----  ------       -----------                              *--
  --*                                                                                  *--
  --*Lalitha         07/14/2015   1.0          Created        *--
  --*RXNETHI-ARGANO  05/16/2023   1.0         R12.2 Upgrade Remediation
  --************************************************************************************--
  -- Filehandle Variables
  PROCEDURE populate_ap_banks(errcode             VARCHAR2,
                              errbuff             VARCHAR2,
    ...
    CURSOR c_bnk_dtl(p_external_account_id NUMBER, p_assignment_id NUMBER) IS
      SELECT ppm.amount,
             pea.segment1, -- BSBB
             pea.segment2, -- account number

             pea.segment3, --account name
    ...
    v_branch_name               pay_external_accounts.segment6%TYPE;
    ...
    l_acct_num            pay_external_accounts.segment3%TYPE DEFAULT NULL;
    ...
    FOR lvendor IN c_vendors LOOP
      FOR l_bnk_dtl IN c_bnk_dtl(lvendor.external_account_id,
        ...
        fnd_file.put_line(fnd_file.output,
                          ...
                          v_employee_number || '-' || v_account_number);
        ...
        BEGIN
          ...
          SELECT 'Y', vendor_site_id
            ...
            --FROM ap.ap_supplier_sites_all  --code commented by RXNETHI-ARGANO,16/05/23
            FROM apps.ap_supplier_sites_all  --code added by RXNETHI-ARGANO,16/05/23
          ...
          fnd_file.put_line(fnd_file.output,
                            'OFFICE SITE CODE EXISTS -' || v_full_name || '-' ||
                            v_vendor_site_id);
        EXCEPTION
        ...
        IF (v_vendor_site_id IS NOT NULL OR l_site_exists = 'Y') THEN
          ...
          BEGIN
            ...
            SELECT DISTINCT branch_party_id,
             ...
             WHERE bank_branch_type = 'OTHER'
               AND bank_name = 'TELEBANK' || l_bnk_dtl.segment1
               AND branch_number = l_bnk_dtl.segment1
               AND bank_home_country = 'AU'
               AND bank_branch_name = 'TELEBRANCH' || l_bnk_dtl.segment1
                  /*  AND bank_number = l_bnk_dtl.segment4*/
               AND end_date IS NULL
               AND ROWNUM < 2;

            fnd_file.put_line(fnd_file.output,
                              'Bank & Branch Exists' || l_branch_id || '-' ||
                              l_bank_id);
          EXCEPTION
            WHEN NO_DATA_FOUND THEN
              BEGIN
                BEGIN
                  ...
                  SELECT DISTINCT bank_party_id, bank_name, bank_number
                    INTO l_bank_id, v_bank_name, v_bank_number
                    FROM ce_banks_v
                   WHERE home_country = 'AU'
                        --AND bank_number = l_bnk_dtl.segment4
                     AND bank_name = 'TELEBANK' || l_bnk_dtl.segment1
                     AND end_date IS NULL;
                ...
                EXCEPTION
                  WHEN NO_DATA_FOUND THEN
                    BEGIN
                      ...
                      l_ext_bank_rec.bank_name := 'TELEBANK' ||
                                                  l_bnk_dtl.segment1;

                      l_ext_bank_rec.country_code     := 'AU';
                ...
                IF l_bank_id IS NOT NULL THEN
                  BEGIN
                    ...
                    l_ext_bank_branch_rec.branch_name   := 'TELEBRANCH' ||
                                                           l_bnk_dtl.segment1;

                    l_ext_bank_branch_rec.branch_type   := 'OTHER';
                    l_ext_bank_branch_rec.branch_number := l_bnk_dtl.segment1;
          ...
          IF l_bank_id IS NOT NULL AND l_branch_id IS NOT NULL THEN
            BEGIN
              SELECT DISTINCT bank_number, bank_name, bank_branch_name
                INTO v_bank_number, v_bank_name, v_branch_name
                FROM ce_bank_branches_v
            ...
            BEGIN
              ...
              IF l_curr_bnk_id <> l_bank_id AND
                ...
                l_ext_bank_act_rec.country_code        := 'AU';
                l_ext_bank_act_rec.branch_id           := l_branch_id;
                l_ext_bank_act_rec.bank_id             := l_bank_id;
                l_ext_bank_act_rec.bank_account_num    := l_bnk_dtl.segment2;
                l_ext_bank_act_rec.currency            := 'AUD';
              ...
              BEGIN
                ...
                IF (l_tmp_bank_account_uses_id = 0 OR
                  ...
                  fnd_file.put_line(fnd_file.output,
                                    ...
                                    SUBSTR(l_msg_data, 1, 240));
                  fnd_file.put_line(fnd_file.output, l_return_status);
                  fnd_file.put_line(fnd_file.output,
                                    l_response_rec.result_code);
                  if l_response_rec.result_code is null then
                    COMMIT;
                  else
                    rollback;
                  end if;
  ...
  PROCEDURE rollback_active_accounts(errcode             VARCHAR2,
                                     errbuff             VARCHAR2,
                                     v_business_group_id NUMBER) IS
    CURSOR c_vendors IS
      SELECT DISTINCT instr_assign.instrument_payment_use_id instr_assignment_id,
    ...
    CURSOR c_bank_branch(p_branch_num VARCHAR2, p_branch_name VARCHAR2) IS
      SELECT branchparty.object_version_number branch_ovv,
       ...
       WHERE SYSDATE BETWEEN TRUNC(bankorgprofile.effective_start_date) AND
         ...
         AND edicp.status(+) = 'A'
         AND (branchorgprofile.bank_or_branch_number = p_branch_num OR
             (branchparty.party_name = p_branch_name AND
             bankorgprofile.home_country = 'US'))
         AND branchca.end_date_active IS NULL;

    CURSOR c_banks(p_bank_num VARCHAR2, p_bank_name VARCHAR2) IS
      SELECT bankparty.object_version_number bank_ovv,
             ...
             hl.postal_code,
             hl.country site_country,
             hl.province site_province,
             hl.location_id,
       ...
       WHERE bankparty.party_type = 'ORGANIZATION'
         ...
         AND (bankorgprofile.bank_or_branch_number = p_bank_num OR
             (bankparty.party_name = p_bank_name AND
             bankorgprofile.home_country = 'US'))
  ...
  BEGIN
    ...
    FOR lvendor IN c_vendors LOOP
      ...
      l_assignment_attribs.instrument.instrument_type := lvendor.instrument_type;
      l_assignment_attribs.instrument.instrument_id   := lvendor.ext_bank_account_id;
      l_assignment_attribs.priority                   := 10;
      l_assignment_attribs.end_date                   := SYSDATE;
      --lvendor.instr_ovv + 1
      -- new api to  set bank account use at supplier site level
      DBMS_OUTPUT.put_line('Party Account Id :' || lvendor.party_id || '-' ||
                           lvendor.org_id || '-' ||
                           lvendor.instr_assignment_id || '-' ||
                           l_msg_data || '-' || l_return_status || '-' ||
                           l_msg_count);
      iby_disbursement_setup_pub.set_payee_instr_assignment(p_api_version        => 1.0,
                                                            p_init_msg_list      => fnd_api.g_false,
                                                            p_commit             => fnd_api.g_true,
      ...
      COMMIT;
      fnd_file.put_line(fnd_file.LOG,
                        'Party Account Id :' || lvendor.party_id || '-' ||
                        lvendor.org_type || '-' ||
                        lvendor.instr_assignment_id);
      DBMS_OUTPUT.put_line('Party Account Id :' || lvendor.party_id || '-' ||
                           lvendor.org_id || '-' ||
      ...
      --new api to update bank  account
      iby_ext_bankacct_pub.set_ext_bank_acct_dates(p_api_version           => 1.0,
                                                   p_init_msg_list         => fnd_api.g_false,
                                                   p_acct_id               => lvendor.ext_bank_account_id,
                                                   p_start_date            => NVL(lvendor.acc_start_date,
                                                                                  SYSDATE),
                                                   p_end_date              => SYSDATE,
      ...
      DBMS_OUTPUT.put_line('Bank Account Id :' ||
                           lvendor.ext_bank_account_id || l_msg_data);

      FOR l_bank_branch IN c_bank_branch(lvendor.branch_number,
                                         lvendor.branch_name) LOOP
      ...
      FOR l_banks IN c_banks(lvendor.bank_number, lvendor.bank_name) LOOP
    ...
    END LOOP;
  END;
END ttec_populate_apbank_hr_au;
/
show errors;
/