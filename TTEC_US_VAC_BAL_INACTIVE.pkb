create or replace PACKAGE BODY      ttec_us_vac_bal_inactive
AS
   ...
   IS
	  ...
	  --      --v1.0 RXNETHI-ARGANO  16May2023    R12.2 Upgrade Remediation
      ...
      l_title           VARCHAR2 (200)
         := 'US Monthly Vacation Balance - InActive Employees - Pay Period End Date: ';
      l_endofmonth      DATE;
      ...
      IS
         /*       Report Name: Finance Report US - Inactive Employees         *
           ...
           FROM (SELECT DISTINCT
                   ...
                   FROM per_all_people_f papf,
                        ...
                        (  SELECT pca1.assignment_id,
                                  ...
                                  proportion
                             --FROM hr.pay_cost_allocations_f pca1  --code commented by RXNETHI-ARGANO,16/05/23
                             FROM apps.pay_cost_allocations_f pca1  --code added by RXNETHI-ARGANO,16/05/23
                            WHERE pca1.effective_end_date =
                                     (SELECT MAX (pca2.effective_end_date)
                                        --FROM hr.pay_cost_allocations_f pca2  --code commented by RXNETHI-ARGANO,16/05/23
                                        FROM apps.pay_cost_allocations_f pca2  --code added by RXNETHI-ARGANO,16/05/23
                                       WHERE pca2.assignment_id =
                                                pca1.assignment_id)
                                  AND l_EndOfMonth BETWEEN pca1.effective_start_date
                                                       AND pca1.effective_end_date
                         GROUP BY pca1.assignment_id,
                                  pca1.cost_allocation_keyflex_id,
                                  proportion) alloc,
                        --hr.pay_cost_allocation_keyflex asg_cost,  --code commented by RXNETHI-ARGANO,16/05/23
                        apps.pay_cost_allocation_keyflex asg_cost,  --code added by RXNETHI-ARGANO,16/05/23
                        apps.fnd_lookup_values fnd_hourly,
                        hr_organization_units org,
                        --hr.pay_cost_allocation_keyflex i_org,   --code commented by RXNETHI-ARGANO,16/05/23
                        apps.pay_cost_allocation_keyflex i_org,   --code added by RXNETHI-ARGANO,16/05/23
   ...
   EXCEPTION
      ...
      THEN
         ...
         raise_application_error (
            -20003,
            'Exception OTHERS in Montly Vacation Balance Program : ' || l_msg);
         ROLLBACK;
   END write_file;
END ttec_us_vac_bal_inactive;
/
show errors;
/