create or replace PACKAGE BODY      ttec_pay_mex_cfdi_2015_intf
IS
   /*---------------------------------------------------------------------------------------
    Objective    : Interface to extract data for all Mexico employees to send to vendor
    Package spec :ttec_pay_mex_cfdi_2015_intf
    Parameters:p_output_dir    --  output directory to generate the files.
	  ...
	  RXNETHI-ARGANO       1.0      5/11/2023  R12.2 Upgrade Remediation
   *== END ==================================================================================================*/
      ...
      SELECT NVL (SUM (a.balance_value), 0)
        ...
        FROM (SELECT prb.assignment_id, prb.balance_value,
                ...
                FROM (SELECT defined_balance_id, assignment_id,
                        ...
                        --FROM hr.pay_run_balances --code commented by RXNETHI-ARGANO,11/05/23
						FROM apps.pay_run_balances --code added by RXNETHI-ARGANO,11/05/23
                       WHERE     effective_date = p_effective_date
                             AND assignment_id IS NOT NULL
                             AND assignment_id = p_assignment_id) prb,
                     --hr.pay_defined_balances pdb --code commented by RXNETHI-ARGANO,11/05/23
					 apps.pay_defined_balances pdb --code added by RXNETHI-ARGANO,11/05/23
               WHERE prb.defined_balance_id = pdb.defined_balance_id) a,
             --hr.pay_balance_types pbt,      --code commented by RXNETHI-ARGANO,11/05/23
             --hr.pay_balance_dimensions pbd  --code commented by RXNETHI-ARGANO,11/05/23
			 apps.pay_balance_types pbt,      --code added by RXNETHI-ARGANO,11/05/23
             apps.pay_balance_dimensions pbd  --code added by RXNETHI-ARGANO,11/05/23
...
-- 1.2 Kaushik
   ...
   BEGIN
      IF p_element_name = 'ISR' or p_class_name = 'Tax Credit'
      THEN
           ...
           FROM (SELECT prr.run_result_id, prr.assignment_action_id,
                ...
                (SELECT paa.assignment_action_id, ppa.date_earned,
                   ...
                   FROM apps.pay_assignment_actions paa,
                        ...
                        --hr.per_all_assignments_f paaf, --code commented by RXNETHI-ARGANO,11/05/23
						apps.per_all_assignments_f paaf, --code added by RXNETHI-ARGANO,11/05/23
...
/*
      ...
      IS
         SELECT org_information1, org_information2, hla.address_line_1,
           ...
           --FROM hr.hr_organization_information hoi, --code commented by RXNETHI-ARGANO,11/05/23
		   FROM apps.hr_organization_information hoi, --code added by RXNETHI-ARGANO,11/05/23
            ...
            LOOP
               ...
               v_second_type :=
                  ...
                  v_net_pay := get_ear_value (NULL,
                     ...
                     select decode(sign( v_net_pay - 0)
                                  ,1 ,v_net_pay,0)
                     into v_net_pay_dsp
                     from dual;

                v_net_perception :=  get_ear_value (NULL,
                                            'Earning',
                                            'ISR Subject',
                                            r_emp_info.assignment_id,
                                            v_cut_off_date,
...
--                          ,'MXN')
               ...
               BEGIN
                  ...
                  v_text :=
                     ...
                     || ''
                     || '|'
                     -- below should be PES amt1 + amt2
                     || get_amount (v_net_perception)
--                     || get_amount (
--                             get_ear_value (NULL,
--                                            'Earning',
--                                            'ISR Subject',
...
--                                            )
   ...
   END main_proc;
END ttec_pay_mex_cfdi_2015_intf;
/
show errors;
/