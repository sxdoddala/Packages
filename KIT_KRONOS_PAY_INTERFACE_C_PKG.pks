create or replace PACKAGE KIT_KRONOS_PAY_INTERFACE_C_PKG IS
 /************************************************************************************
        Program Name: KIT_KRONOS_PAY_INTERFACE_C_PKG 
...
FROM
 --  KRONOS.KSS_PAYROLL_ORACLE KPO,  --Commented code by MXKEERTHI-ARGANO,07/17/2023
  ...
  WHERE ELEMENT_NAME IN (SELECT ELEMENT_NAME FROM PAY_ELEMENT_TYPES_F
                        where business_group_id = 326) AND
    RECORD_TRACK ='L' AND
    DELETIONINDICATOR=0
  GROUP BY
    BATCH_NAME,
...
WHERE ELEMENT_NAME IN (SELECT ELEMENT_NAME FROM PAY_ELEMENT_TYPES_F where business_group_id = 326) AND
...
PROCEDURE KIT_KRONOS_PAYROLL_C_INTERFACE   ;

--Procedure for Logging the errors.
--PROCEDURE Log_Error;

END KIT_KRONOS_PAY_INTERFACE_C_PKG;
/
show errors;
/