create or replace PACKAGE BODY      ttec_pay_mex_cfdi_2015_cp_intf
IS
   /*---------------------------------------------------------------------------------------
    Objective    : Interface to extract data for all Mexico employees to send to vendor
    Package spec :ttec_pay_mex_cfdi_2015_cp_intf
    Parameters:p_output_dir    --  output directory to generate the files.
      ...
      Christiane Chan      1.15     8/11/2016  Mexico CFDI H1 record update
      ...
      SELECT NVL (SUM (a.balance_value), 0)
        ...
        FROM (SELECT prb.assignment_id, prb.balance_value,
                ...
                FROM (SELECT defined_balance_id, assignment_id,
                        ...
                        --FROM hr.pay_run_balances		-- Commented code by IXPRAVEEN-ARGANO, 04-May-2023
						FROM apps.pay_run_balances		--  code Added by IXPRAVEEN-ARGANO, 04-May-2023
                       WHERE     effective_date = p_effective_date
                             AND assignment_id IS NOT NULL
                             AND assignment_id = p_assignment_id) prb,
                     --hr.pay_defined_balances pdb		-- Commented code by IXPRAVEEN-ARGANO, 04-May-2023
					 apps.pay_defined_balances pdb		--  code Added by IXPRAVEEN-ARGANO, 04-May-2023
               WHERE prb.defined_balance_id = pdb.defined_balance_id) a,
			   --START R12.2 Upgrade Remediation
             /*hr.pay_balance_types pbt,		-- Commented code by IXPRAVEEN-ARGANO, 04-May-2023
             hr.pay_balance_dimensions pbd*/
			 apps.pay_balance_types pbt,		--  code Added by IXPRAVEEN-ARGANO, 04-May-2023
           ...
           FROM (SELECT prr.run_result_id, prr.assignment_action_id,
                ...
                (SELECT paa.assignment_action_id, ppa.date_earned,
                   ...
                   FROM apps.pay_assignment_actions paa,
                        ...
                        --hr.per_all_assignments_f paaf,			-- Commented code by IXPRAVEEN-ARGANO, 04-May-2023
						apps.per_all_assignments_f paaf,		--  code Added by IXPRAVEEN-ARGANO, 04-May-2023	
   ...
   IS
      ...
      IS
           SELECT DISTINCT ftt.territory_short_name country,
                           ...
                           papf.EMAIL_ADDRESS||';'||papf.ATTRIBUTE1 email_address, /*1.14 */
            ...
            /* FROM hr.per_all_people_f papf,
			 ...
			 FROM apps.per_all_people_f papf,
                  ...
                  apps.pay_cost_allocation_keyflex pcak_org,		--  code Added by IXPRAVEEN-ARGANO, 04-May-2023
                  ...
                  --hr.per_pay_proposals ppp,			-- Commented code by IXPRAVEEN-ARGANO, 04-May-2023
				  apps.per_pay_proposals ppp,				--  code Added by IXPRAVEEN-ARGANO, 04-May-2023
                  ...
                  --hr.per_assignment_status_types past,		-- Commented code by IXPRAVEEN-ARGANO, 04-May-2023
				  apps.per_assignment_status_types past,			--  code Added by IXPRAVEEN-ARGANO, 04-May-2023
                  --cust.ttec_emp_proj_asg tepa,				-- Commented code by IXPRAVEEN-ARGANO, 04-May-2023
                  apps.ttec_emp_proj_asg tepa,                  	--  code Added by IXPRAVEEN-ARGANO, 04-May-2023
         ...
         SELECT DISTINCT
           ...
           FROM (SELECT prr.run_result_id, prr.assignment_action_id,
                  ...
                  WHERE prr.element_type_id = petf.element_type_id
                        ...
                        AND pec.classification_name LIKE
                               '%' || p_class_name || '%'
                        AND petf.element_name = 'MX_GROSERY_COUPONS') a,   --1.10
--                        AND petf.element_name NOT IN
--                               ('MX_ML_WORK_RISK',
--                                'MX_ML_SICKNESS',
--                                'MX_ML_MATERNITY',
...
/*
      ...
      IS
         SELECT org_information1, org_information2, hla.address_line_1,
           ...
           --FROM hr.hr_organization_information hoi,		-- Commented code by IXPRAVEEN-ARGANO, 04-May-2023
		   FROM apps.hr_organization_information hoi,		--  code Added by IXPRAVEEN-ARGANO, 04-May-2023
            ...
            LOOP
               ...
               BEGIN
                  v_text :=
                     ...
                     || ''
                     || '|'
                     || r_emp_info.H1_F30 --'MEXICO DF'
                     || '|'
                     || 'Monedero Electrónico' /* 1.15 */
                     || '|'
                     || 'MXN'
                     || '|'
                     || ''
...
--                                               )
   ...
   END main_proc;
END ttec_pay_mex_cfdi_2015_cp_intf;
/
show errors;
/
