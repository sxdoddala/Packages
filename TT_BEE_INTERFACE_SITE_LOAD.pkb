create or replace package body      TT_BEE_INTERFACE_SITE_LOAD is

/************************************************************************************/
/*                                                                                  */
/*     Program Name: TT_BEE_INTERFACE_SITE_LOAD  pkg                             */
/*                                                                                  */
/*     Description:  This package loads BEE data                                                       */
/*                                                                                  */
/*     Input/Output Parameters:                                                     */
/*                                                                                  */
/*     Tables Accessed:  apps.fnd_common_lookups                        */
/*                                hr.pay_element_types_f                */
/*                                hr.pay_input_values_f                           */
/*                               cust.tt_bee_interface_stage                     */
/*                              hr.per_all_people_f                             */
...
/*                                                                                  */
/*     Procedures Called:        get_unprocessed_batch_count                     */
/*                                          get_batch_id                    */
/*                                          get_batch_line_id               */
/*                                          get_assignment_info             */
/*                                          get_employee_name           */
...
/*     ----------     --------------------   --------  --------------------------------            */
/*     1.1              J. Keener               1/14/11         R12 Retrofit: Increased file handle             */
/*                                                                                          variable size under the         */
/*                                                                                          generate_audit_report procedure
/*     1.0              RXNETHI-ARGANO          29-JUN-23       R12.2 Upgrade Remediation   */
/************************************************************************************/
  ...
  /************************************************************************************/
    PROCEDURE get_assignment_info(p_assignment_number IN VARCHAR2
                                 ...
                                 ,p_employee_name OUT VARCHAR2) IS
    ...
    BEGIN
      ...
      SELECT b.assignment_id, a.employee_number, b.payroll_id, a.full_name
      INTO p_assignment_id, p_employee_number, p_payroll_id,p_employee_name
      ...
      AND b.assignment_number = p_assignment_number;
  ...
  /************************************************************************************/
    ...
    BEGIN
      ...
      INTO apps.pay_batch_lines(batch_line_id, element_type_id, assignment_id, batch_id  --code added by RXNETHI-ARGANO,29/06/23
        ...
        ,NVL(p_effective_start_date,p_effective_date),p_effective_end_date);
  ...
  /*                               DELETE_STAGE_TABLE                                 */
  /************************************************************************************/
    PROCEDURE delete_stage_table(p_site_name IN VARCHAR2 DEFAULT NULL) IS

    BEGIN
      -- set global module name for error handling
        g_module_name := 'delete_stage_table';
        g_label2 := NULL;
        g_secondary_column := NULL;

    --DELETE FROM cust.tt_bee_interface_stage   --code commented by RXNETHI-ARGANO,29/06/23
    DELETE FROM apps.tt_bee_interface_stage     --code added by RXNETHI-ARGANO,29/06/23
  ...
  /************************************************************************************/
    ...
    CURSOR csr_bee_tble_data_loc IS
      SELECT hla.location_code, pbl.element_name, pbl.element_type_id
		   ...
		   ,sum(DECODE(cust.TT_BEE_INTERFACE_SITE_LOAD.is_number(pbl.value_1), 'TRUE'
                ,to_number(pbl.value_1), 0)) "VALUE_1"
           ,sum(DECODE(cust.TT_BEE_INTERFACE_SITE_LOAD.is_number(pbl.value_2), 'TRUE'
                ,to_number(pbl.value_2), 0)) "VALUE_2"
           ,sum(DECODE(cust.TT_BEE_INTERFACE_SITE_LOAD.is_number(pbl.value_3), 'TRUE'
                ,to_number(pbl.value_3), 0)) "VALUE_3"
           ,sum(DECODE(cust.TT_BEE_INTERFACE_SITE_LOAD.is_number(pbl.value_4), 'TRUE'
                ,to_number(pbl.value_4), 0)) "VALUE_4"
           ,sum(DECODE(cust.TT_BEE_INTERFACE_SITE_LOAD.is_number(pbl.value_5), 'TRUE'
                ,to_number(pbl.value_5), 0)) "VALUE_5"
      FROM hr.pay_batch_headers pba, hr.pay_batch_lines pbl, hr.per_all_assignments_f paaf
          ,hr.hr_locations_all hla
		  */
		  --code added by RXNETHI-ARGANO,29/06/23
		  ,sum(DECODE(apps.TT_BEE_INTERFACE_SITE_LOAD.is_number(pbl.value_1), 'TRUE'
                ,to_number(pbl.value_1), 0)) "VALUE_1"
           ,sum(DECODE(apps.TT_BEE_INTERFACE_SITE_LOAD.is_number(pbl.value_2), 'TRUE'
                ,to_number(pbl.value_2), 0)) "VALUE_2"
           ,sum(DECODE(apps.TT_BEE_INTERFACE_SITE_LOAD.is_number(pbl.value_3), 'TRUE'
                ,to_number(pbl.value_3), 0)) "VALUE_3"
           ,sum(DECODE(apps.TT_BEE_INTERFACE_SITE_LOAD.is_number(pbl.value_4), 'TRUE'
                ,to_number(pbl.value_4), 0)) "VALUE_4"
           ,sum(DECODE(apps.TT_BEE_INTERFACE_SITE_LOAD.is_number(pbl.value_5), 'TRUE'
                ,to_number(pbl.value_5), 0)) "VALUE_5"
      FROM apps.pay_batch_headers pba, apps.pay_batch_lines pbl, apps.per_all_assignments_f paaf
  ...
  /************************************************************************************/
    ...
    CURSOR csr_bee_tble_data_sum IS
      SELECT pbl.element_name, pbl.element_type_id
           ...
           ,sum(DECODE(cust.TT_BEE_INTERFACE_SITE_LOAD.is_number(pbl.value_5), 'TRUE'
                ,to_number(pbl.value_5), 0)) "VALUE_5"
      FROM hr.pay_batch_headers pba, hr.pay_batch_lines pbl
	  */
	  --code added by RXNETHI-ARGANO,29/06/23
	  ,sum(DECODE(apps.TT_BEE_INTERFACE_SITE_LOAD.is_number(pbl.value_1), 'TRUE'
                ,to_number(pbl.value_1), 0)) "VALUE_1"
           ...
           ,sum(DECODE(apps.TT_BEE_INTERFACE_SITE_LOAD.is_number(pbl.value_5), 'TRUE'
                ,to_number(pbl.value_5), 0)) "VALUE_5"
      FROM apps.pay_batch_headers pba, apps.pay_batch_lines pbl
	  --END R12.2 Upgrade Remediation
      WHERE pba.batch_id = pbl.batch_id
      AND pba.batch_name = p_batch_name
      AND pba.batch_reference = p_batch_reference
    ...
    BEGIN
       ...
       FOR sel IN csr_bee_tble_data_sum LOOP
          ...
          apps.fnd_file.put(2,rpad(nvl(get_element_value_name(sel.element_name, sel.element_type_id
            , 1,sysdate), ' '),14));
             g_module_name := 'output_bee_summary1';
          apps.fnd_file.put(2,' ');
          apps.fnd_file.put(2,lpad(nvl(to_char(sel.value_1),' ' ),12));
          apps.fnd_file.put(2,' ');
          apps.fnd_file.put(2,rpad(nvl(get_element_value_name(sel.element_name, sel.element_type_id
            , 2,sysdate), ' '),14));
             g_module_name := 'output_bee_summary2';
          apps.fnd_file.put(2,' ');
          apps.fnd_file.put(2,lpad(nvl(to_char(sel.value_2),' ' ),12));
          apps.fnd_file.put(2,' ');
          apps.fnd_file.put(2,rpad(nvl(get_element_value_name(sel.element_name, sel.element_type_id
            , 3,sysdate), ' '),14));
             g_module_name := 'output_bee_summary3';
          apps.fnd_file.put(2,' ');
          apps.fnd_file.put(2,lpad(nvl(to_char(sel.value_3),' ' ),12));
          apps.fnd_file.put(2,' ');
          apps.fnd_file.put(2,rpad(nvl(get_element_value_name(sel.element_name, sel.element_type_id
            , 4,sysdate), ' '),14));
             g_module_name := 'output_bee_summary4';
          apps.fnd_file.put(2,' ');
          apps.fnd_file.put(2,lpad(nvl(to_char(sel.value_4),' ' ),12));
          apps.fnd_file.put(2,' ');
          apps.fnd_file.put(2,rpad(nvl(get_element_value_name(sel.element_name, sel.element_type_id
            , 5,sysdate), ' '),14));
             g_module_name := 'output_bee_summary5';
          apps.fnd_file.put(2,' ');
          g_module_name := 'output_bee_summary5.1';
          apps.fnd_file.put(2,lpad(nvl(to_char(sel.value_5),' ' ),12));
          g_module_name := 'output_bee_summary5.2';
          apps.fnd_file.put_line(2,' ');
          g_module_name := 'output_bee_summary6';
       END LOOP;
       g_module_name := 'output_bee_summary7';
       apps.fnd_file.put_line(2,'');
       g_module_name := 'output_bee_summary8';
       apps.fnd_file.put_line(2,'-----------------------------------------------------------------------------');
       g_module_name := 'output_bee_summary9';
       apps.fnd_file.put_line(2,'---------------------------------- REPORT END  ------------------------------');
       apps.fnd_file.put_line(2,'output_bee_summary stage 1');
       apps.fnd_file.put_line(2,'-----------------------------------------------------------------------------');
       apps.fnd_file.put_line(2,'output_bee_summary stage 2');
       apps.fnd_file.put_line(2,'');
       apps.fnd_file.put_line(2,'output_bee_summary stage 3');
       apps.fnd_file.put_line(2,'NOTE:  IF THE VALUES DO NOT RECONCILE, PLEASE CHECK THE VALUES IN THE BATCH.');
       apps.fnd_file.put_line(2,'output_bee_summary stage 4');
       apps.fnd_file.put_line(2,'       THERE MAY BE SOME VALUES THAT ARE NOT NUMERIC. NON-NUMERIC VALUES ARE ');
       apps.fnd_file.put_line(2,'output_bee_summary stage 5');
       apps.fnd_file.put_line(2,'       ACCOUNTED FOR AS ZERO VALUES.');
       apps.fnd_file.put_line(2,'output_bee_summary stage 6');
       apps.fnd_file.put_line(2,'');
       apps.fnd_file.put_line(2,'output_bee_summary stage 7');
    EXCEPTION
             WHEN OTHERS THEN
               apps.fnd_file.put_line(2,'output_bee_summary stage 10');
               apps.fnd_file.put_line(1,'ERROR:  COULD NOT COMPLETE THE PRODUCTION OF THE RECONCILIATION REPORT.');
               apps.fnd_file.put_line(2,'output_bee_summary stage 11');
               apps.fnd_file.put_line(1,'        PLEASE CONTACT THE SYSTEM ADMINISTRATOR TO HELP DETERMINE');
               apps.fnd_file.put_line(1,'        THE ERROR.');
               apps.fnd_file.put_line(1,'');
               apps.fnd_file.put_line(1,'NOTE:   BATCH HAS BEEN LOADED INTO THE BEE TABLES.');
  ...
  /************************************************************************************/
    PROCEDURE generate_audit_report(P_SITE_NAME IN VARCHAR2, P_OUTPUT_DIR IN VARCHAR2) IS
    -- Filehandle Variables
    p_FileDir VARCHAR2(100) := P_OUTPUT_DIR;                                                        -- 1.1
    p_FileName VARCHAR2(100):= 'BEE_LOAD_AUDIT_'||REPLACE(P_SITE_NAME,' ','')||'.csv';          -- 1.1
    v_daily_file UTL_FILE.FILE_TYPE;

    -- Declare variables
    l_rec     VARCHAR2(5000);                                                                                       -- 1.1
	...
	START R12.2 Upgrade Remediation
	code commented by RXNETHI-ARGANO,29/06/23
	,sum(DECODE(cust.TT_BEE_INTERFACE_SITE_LOAD.is_number(value1), 'TRUE'
               ,to_number(value1), 0)) "TOTAL_VALUE_1"
    ,sum(DECODE(cust.TT_BEE_INTERFACE_SITE_LOAD.is_number(value2), 'TRUE'
                ,to_number(value2), 0)) "TOTAL_VALUE_2"
    ,sum(DECODE(cust.TT_BEE_INTERFACE_SITE_LOAD.is_number(value3), 'TRUE'
                ,to_number(value3), 0)) "TOTAL_VALUE_3"
    ,sum(DECODE(cust.TT_BEE_INTERFACE_SITE_LOAD.is_number(value4), 'TRUE'
                ,to_number(value4), 0)) "TOTAL_VALUE_4"
    ,sum(DECODE(cust.TT_BEE_INTERFACE_SITE_LOAD.is_number(value5), 'TRUE'
                ,to_number(value5), 0)) "TOTAL_VALUE_5"
    from cust.tt_bee_interface_stage
	*/
	--code added by RXNETHI-ARGANO,29/06/23
	,sum(DECODE(apps.TT_BEE_INTERFACE_SITE_LOAD.is_number(value1), 'TRUE'
               ,to_number(value1), 0)) "TOTAL_VALUE_1"
    ,sum(DECODE(apps.TT_BEE_INTERFACE_SITE_LOAD.is_number(value2), 'TRUE'
                ,to_number(value2), 0)) "TOTAL_VALUE_2"
    ,sum(DECODE(apps.TT_BEE_INTERFACE_SITE_LOAD.is_number(value3), 'TRUE'
                ,to_number(value3), 0)) "TOTAL_VALUE_3"
    ,sum(DECODE(apps.TT_BEE_INTERFACE_SITE_LOAD.is_number(value4), 'TRUE'
                ,to_number(value4), 0)) "TOTAL_VALUE_4"
    ,sum(DECODE(apps.TT_BEE_INTERFACE_SITE_LOAD.is_number(value5), 'TRUE'
                ,to_number(value5), 0)) "TOTAL_VALUE_5"
    from apps.tt_bee_interface_stage
	--END R12.2 Upgrade Remediation
    where SITE_NAME = P_SITE_NAME
    and RECORD_PROCESSED = 'Y'
    and ERROR_FLAG = 'N'
    group by BUSINESS_GROUP,site_name,Batch_name,bee_effective_date,value3;

    BEGIN
       v_daily_file := UTL_FILE.FOPEN(p_FileDir, p_FileName, 'w');
       l_rec := 'Report Name : TeleTech Payroll - Global Bee Interface Data Load Verification';
       utl_file.put_line(v_daily_file, l_rec);
       l_rec := 'Report Date : '||SYSDATE;
       utl_file.put_line(v_daily_file, l_rec);
       l_rec := 'Site Name   : '||P_SITE_NAME;
       utl_file.put_line(v_daily_file, l_rec);
       l_rec := '';
       utl_file.put_line(v_daily_file, l_rec);
       l_rec := '';
       ...
       l_rec := '';
       utl_file.put_line(v_daily_file, l_rec);
       --l_rec := 'Site Name,Batch Name,BEE Effective Date,Total Rows,Total Value1,Total Value2,Total Value3,Total Value4,Total Value5,';
                   l_rec := rpad('Batch Name',30,' ')
                    ||' '|| rpad('Eff. Date',9,' ')
                    ||' '|| lpad('Rows',5,' ')
                    ||' '|| lpad('Value1',10,' ')
                    ||' '|| lpad('Value2',10,' ')
       ...
       utl_file.put_line(v_daily_file, l_rec);
                   l_rec := rpad('-',30,'-')
                    ...
                    ||' '|| lpad('-',10,'-');
       utl_file.put_line(v_daily_file, l_rec);
   --    apps.fnd_file.put_line(apps.fnd_file.output,'Report Name : TeleTech Payroll - Global Bee Interface Data Load Verification');
   --    apps.fnd_file.put_line(apps.fnd_file.output,'Report Date : '||SYSDATE);
   --   apps.fnd_file.put_line(apps.fnd_file.output,'Site Name   : '||P_SITE_NAME);
   --    apps.fnd_file.new_line(apps.fnd_file.output,3);
   --   apps.fnd_file.put_line(apps.fnd_file.output,'Site Name,Batch Name,BEE Effective Date,Total Rows,Total Amount,');
   --    apps.fnd_file.new_line(apps.fnd_file.output,2);
       for rec in c_summary loop
            l_rec :=        rpad(substr(rec.batch_name,1,30),30,' ')
                    ||' '|| rpad(substr(rec.bee_effective_date,1,9),9,' ')
                    ||' '|| lpad(rec.total_rows,5,' ')
                    ||' '|| lpad(rec.total_value_1,10,' ')
                    ||' '|| lpad(rec.total_value_2,10,' ')
                    ||' '|| lpad(rec.total_value_3,10,' ')
                    ||' '|| lpad(rec.total_value_4,10,' ')
                    ||' '|| lpad(rec.total_value_5,10,' ');
           utl_file.put_line(v_daily_file, l_rec);
    --       apps.fnd_file.put_line(apps.fnd_file.output,rec.site_name
    --                ||','|| rec.batch_name
    --                ||','|| rec.bee_effective_date
    --                ||','|| rec.total_rows
    --                ||','|| rec.total_amount
    --                );
      End Loop;
  ...
  /************************************************************************************/
    ...
    PROCEDURE main (ERRBUF OUT VARCHAR2, RETCODE OUT NUMBER, l_parameter_business_group_id IN NUMBER,
                                                             ...
                                                             l_output_directory IN VARCHAR2) IS
...
apps.fnd_file.put_line(1,'');
         ...
         begin
           select name into g_param_business_group_name
              --from hr_organization_units  -- Commented code by RXNETHI-ARGANO, 29-JUN-23 
              from apps.hr_organization_units  -- Added code by RXNETHI-ARGANO, 29-JUN-23 
         ...
         FOR sel_loop1 IN csr_stage_unprocessed_batches(g_param_business_group_name,l_site_name) LOOP
            ...
            apps.fnd_file.put_line(1,'Start Batch: '||g_batch_name||' /Business Group Name: '||l_business_group_name||' /Site Name: '||l_site_name);

            -- output header
            apps.fnd_file.put_line(2,'***********************************************************************************************************************');
            apps.fnd_file.put_line(2,'START OF BATCH: '||g_batch_name);
            apps.fnd_file.put_line(2,'BUSINESS GROUP: '||l_business_group_name);
            apps.fnd_file.put_line(2,'SITE NAME     : '||l_site_name);
            apps.fnd_file.put_line(2,'');
...
apps.fnd_file.put_line(1,'Batch Header Record Created: Batch ID = '||l_batch_id);
            ...
            apps.fnd_file.put_line(2,'-------------------------------------');
            apps.fnd_file.put_line(2,'');
            apps.fnd_file.put_line(2,'Employee Name        Ass Num Element Name      Value1      Value2      Bee Eff.Dt  Eff.Strt.Dt Eff.End Dt. Error Message');
            apps.fnd_file.put_line(2,'----------------- ---------- ----------------- ----------- ----------- ----------- ----------- ----------- ----------------------------------------------------------------------------------------------------');
                 -- retrieve batch data from cursor
            FOR sel_loop2 IN csr_bee_int_stage_data(g_batch_name, l_business_group_id, g_param_business_group_name,l_site_name) LOOP
               BEGIN
                  ...
                  l_element_effective_date  := sel_loop2.bee_effective_date;
                  l_batch_sequence := l_batch_sequence +1;
                        g_error_message := NULL;
                     g_module_name := 'main_loop2';
                  g_primary_column := sel_loop2.assignment_number;

                  l_assignment_number := sel_loop2.assignment_number;
                 ...
                 -- get assignment information
                  get_assignment_info(l_assignment_number
                                     ,l_business_group_id
                                     ,l_element_effective_date
                                     ,l_assignment_id
                                     ,l_employee_number
                                     ,l_payroll_id
                                     ,l_employee_name);
                  ...
                  -- get employee name
           --       get_employee_name(l_employee_number, l_business_group_id
           --                        ,l_element_effective_date, l_employee_name);
                 ...
                 IF sel_loop2.element_name IN (c_vacation_taken, c_sick_taken
                    ...
                    create_bee_line(l_batch_id, l_batch_line_id, l_element_type_id
                      ...
                      ,sel_loop2.value2, sel_loop2.value3
                      ...
                      ,sel_loop2.effective_start_date,sel_loop2.effective_end_date);-- Added by C. Chan WO#407423
                    g_module_name := 'main_loop2';
                  ELSE
                    -- create bee line
                    create_bee_line(l_batch_id, l_batch_line_id, l_element_type_id
                  ...
                  update_stage_table(sel_loop2.rowid, c_processed_trsfr_flag,'N');
                    g_module_name := 'main_loop2';
               ...
               EXCEPTION
                    WHEN SKIP_RECORD THEN
                       ...
                       update_stage_table(sel_loop2.rowid, c_processed_trsfr_flag,'Y'
                         ,g_error_message);
                       apps.fnd_file.put_line(2,rpad(nvl(l_employee_name,' '),17)
                         ||' '||lpad(nvl(l_assignment_number,' '),10)
                         ||' '||rpad(nvl(sel_loop2.element_name, ' '),17)
                         ||' '||lpad(nvl(sel_loop2.value1,' '),11)
                         ||' '||lpad(nvl(sel_loop2.value2,' '),11)
                         ||' '||rpad(nvl(to_char(sel_loop2.BEE_effective_date,'DD-MON-YYYY'),' '),11)
                         ||' '||rpad(nvl(to_char(sel_loop2.Effective_start_date,'DD-MON-YYYY'),' '),11)
                         ||' '||rpad(nvl(to_char(sel_loop2.effective_end_date,'DD-MON-YYYY'),' '),11)
                         ||' '||rpad(g_error_message, 150));
...
apps.fnd_file.put_line(1,'Outputing unprocessed records due to invalid assignment numbers.');
         -- OUTPUT RECORDS THAT HAVE NOT BEEN PROCESSED DUE TO INVALID EMPLOYEE NUMBERS
         apps.fnd_file.put_line(2,'');
         apps.fnd_file.put_line(2,'UNPROCESSED RECORDS DUE TO INVALID ASSIGNMENT NUMBERS');
         apps.fnd_file.put_line(2,'-----------------------------------------------------');
         apps.fnd_file.put_line(2,'');
         apps.fnd_file.put_line(2,'Batch Name        Ass Num Element Name         Value1      Value2      Value3      Value4      Value5');
         apps.fnd_file.put_line(2,'--------------- --------- -------------------- ----------- ----------- ----------- ----------- -----------');
         ...
         FOR sel_unprocess IN csr_unprocessed_records(g_param_business_group_name,l_site_name) LOOP
            l_unprocessed_records := l_unprocessed_records +1;
            apps.fnd_file.put_line(2,rpad(sel_unprocess.batch_name, 15)||' '||
                                     rpad(sel_unprocess.assignment_number,10)||' '||
                                     rpad(sel_unprocess.element_name,20)||' '||
                                     rpad(sel_unprocess.value1,11)||' '||
                                     rpad(sel_unprocess.value2,11)||' '||
                                     rpad(sel_unprocess.value3,11)||' '||
                                     rpad(sel_unprocess.value4,11)||' '||
                                     rpad(sel_unprocess.value5,11));
            -- error records with invalid employee numbers
            update_stage_table(sel_unprocess.rowid,'Y','Y','Invalid Assignment Number');
...
apps.fnd_file.put_line(1,'Outputing unprocessed records due to invalid employee numbers COMPLETE.');
         ...
         IF l_records_errored > 0 AND l_unprocessed_records <> 0 THEN

            l_completion_code := apps.fnd_concurrent.set_completion_status('ERROR', 'Errors produced during program execution and there are unprocessed'
               ||' records.  Check Output File');
         ELSIF l_records_errored > 0 THEN

            l_completion_code := apps.fnd_concurrent.set_completion_status('ERROR', 'Errors produced during program execution.  Check Output File');
         ELSIF l_unprocessed_records <> 0 THEN

            l_completion_code := apps.fnd_concurrent.set_completion_status('ERROR', 'Program completed, however, there were unprocessed records.'
               ||' Check Output File');
         ELSIF l_records_errored = 0 THEN

            l_completion_code := apps.fnd_concurrent.set_completion_status('NORMAL', 'Program completed successfully without errors.');

            generate_audit_report(l_site_name,l_output_directory);
         ...
         delete_stage_table(l_site_name);
