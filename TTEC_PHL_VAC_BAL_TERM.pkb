create or replace PACKAGE BODY      ttec_phl_vac_bal_term
AS
-- Program Name:   "PHL Monthly Vacation Balance - Termed  Employees"
--
-- Description:
...
-- RXNETHI-ARGANO   10/MAY/23    R12.2 Upgrade Remediation
-- ----------       --------    --------------------------------------------------------------------
...
-- Declare variables
      ...
      l_title           VARCHAR2 (200)
         := 'PHL Monthly Vacation Balance - Termed Employees - Pay Period End Date: ';
      l_endofmonth      DATE;
      ...
      IS
         SELECT    "Business Group"
           ...
           FROM (SELECT DISTINCT 'PHL' "Business Group",
                                 ...
                                 NULL "Vacation Dollars", job.NAME "Job",
                            ...
                            FROM per_all_people_f papf,
                                 ...
                                 (SELECT   pca1.assignment_id,
                                           ...
                                           proportion
                                      --FROM hr.pay_cost_allocations_f pca1 --code commented by RXNETHI-ARGANO,10/05/23
									  FROM apps.pay_cost_allocations_f pca1 --code added by RXNETHI-ARGANO,10/05/23
                                     WHERE pca1.effective_end_date =
                                              (SELECT MAX
                                                         (pca2.effective_end_date
                                                         )
                                                 --FROM hr.pay_cost_allocations_f pca2 --code commented by RXNETHI-ARGANO,10/05/23
                                                FROM apps.pay_cost_allocations_f pca2 --code added by RXNETHI-ARGANO,10/05/23
												WHERE pca2.assignment_id =
                                                            pca1.assignment_id)
                                  GROUP BY pca1.assignment_id,
                                           pca1.cost_allocation_keyflex_id,
                                           proportion) alloc,
                                 --hr.pay_cost_allocation_keyflex asg_cost, --code commented by RXNETHI-ARGANO,10/05/23
								 apps.pay_cost_allocation_keyflex asg_cost, --code added by RXNETHI-ARGANO,10/05/23
                                 hr_organization_units org,
                                 --hr.pay_cost_allocation_keyflex i_org, --code commented by RXNETHI-ARGANO,10/05/232
                                 apps.pay_cost_allocation_keyflex i_org, --code added by RXNETHI-ARGANO,10/05/232
