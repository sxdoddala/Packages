 /************************************************************************************
    ...
    ****************************************************************************************/
create or replace PACKAGE ttec_taleo_perfo_initial_cr
IS
...
--              paaf.ass_attribute30 perfornmance_req,
                ...
                --  AND papf.business_group_id = 325
                  ...
                  AND sysdate BETWEEN papf.effective_start_date  -- will be set to sysdate in program
                                          AND papf.effective_end_date
                  AND sysdate BETWEEN paaf.effective_start_date
                                          AND paaf.effective_end_date
                  AND sysdate BETWEEN papf_sup.effective_start_date
                                          AND papf_sup.effective_end_date
--                 AND  (   (papf.last_update_date between  v_last_run_date and sysdate)
...
--                       )
   ...
   IS
      SELECT      IDENTIFIER
               ...
               || delimit27
               || manager_level l_out
          --FROM cust.ttec_taleoperf_empcreate_db		-- Commented code by IXPRAVEEN-ARGANO,18-july-2023
          FROM apps.ttec_taleoperf_empcreate_db         --  code Added by IXPRAVEEN-ARGANO,   18-july-2023
      ORDER BY last_name;
   ...
   IS
      SELECT    '/d01/ora'
             ...
             || 'appl/teletech/11.5.0/data/BenefitInterface/taleo_performance'
                                                               directory_path,
                'TTEC_TALEO_PERFORM_ONET_CREATE'
