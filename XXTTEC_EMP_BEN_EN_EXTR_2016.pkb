create or replace PACKAGE BODY XXTTEC_EMP_BEN_EN_EXTR_2016
 /********************************************************************************
...
1.0   RXNETHI-ARGANO    03-MAY-2023   R12.2 Upgrade Remediation
  ********************************************************************************/
...
AND TRUNC(p_date) BETWEEN PAP3.EFFECTIVE_START_DATE AND EFFECTIVE_END_DATE
AND PAA.PERSON_ID = PAP.PERSON_ID),0,'A','B') FROM DUAL
),'A','CHOICE001','CHOICE002'),
247,'METLIFE48',246,'METLIFE47',4871,'DELTA07',262,'DELTA06',

272,'METLIFE46',258,'METLIFE45',254,'METLIFE50',256,'METLIFE53',260,'METLIFE52',250,'METLIFE51',252,'METLIFE49',264,'VSP008',277,'METLIFE53',278,'METLIFE51'
,
7871,'TELE001',
275,'TELE002',
7873,'TELE003',
...
7875,'PPL010'
       ...
       DECODE(bpl.pl_id, 260,pen.bnft_amt,245,pen.bnft_amt,248,pen.bnft_amt,256,pen.bnft_amt,277,pen.bnft_amt,247,(SELECT TRUNC((PROPOSED_SALARY_N * 2080),3)
                                                                                       ...
                                                                                       AND TRUNC(p_date) BETWEEN PPP.change_date AND PPP.date_to--added on 11/20/2015
       ...
       -- pen.effective_start_date effective_date,
        ...
        --FROM ben.ben_prtt_rt_val rates --code commented by RXNETHI-ARGANO, 03/MAY/2023
        FROM apps.ben_prtt_rt_val rates   --code added by RXNETHI-ARGANO, 03/MAY/2023
		WHERE rates.prtt_enrt_rslt_id = pen.prtt_enrt_rslt_id
        AND TRUNC(p_date) BETWEEN RT_STRT_DT AND RT_END_DT) ded_start_date,
        DECODE(PEN.PL_ID,9871,DECODE(SUBSTR(OPT.NAME,1,8),'Post Tax','N','Y'),
        11871,DECODE(SUBSTR(OPT.NAME,1,8),'Post Tax','N','Y'),
        10871,DECODE(SUBSTR(OPT.NAME,1,8),'Post Tax','N','Y'),
        247,'N',246,'N',4871,DECODE(SUBSTR(opt.name,1,8),'Post Tax','N','Y'),262,DECODE(SUBSTR(opt.name,1,8),'Post Tax','N','Y'),248,'Y',275,'N',245,'Y',272,'N',258,'N',254,'N',256,'N',277,'N',260,'N',250,'N',278,'N',252,'N',264,DECODE(SUBSTR(opt.name,1,8),'Post Tax','N','Y'),263,'N',249,'N',276,'N',273,'N',268,'N',259,'N',255,'N',257,'N',261,'N',251,'N',253,'N',265,'N') sec_125,
        dep_social_num,
        dep_cont_f_name,
        dep_cont_l_name,
        dep_contact_type,
        dep_dob,
        pen.pl_id
--FROM   hr.per_all_people_f pap,      --code commented by RXNETHI-ARGANO, 03/MAY/2023
--       ben.BEN_PRTT_ENRT_RSLT_F pen, --code commented by RXNETHI-ARGANO, 03/MAY/2023
FROM   apps.per_all_people_f pap,        --code added by RXNETHI-ARGANO, 03/MAY/2023
       apps.BEN_PRTT_ENRT_RSLT_F pen,   --code added by RXNETHI-ARGANO, 03/MAY/2023
       (SELECT *
       --FROM ben.ben_pl_f --code commented by RXNETHI-ARGANO, 03/MAY/2023
       FROM apps.ben_pl_f  --code added by RXNETHI-ARGANO, 03/MAY/2023
	   WHERE trunc(p_date) BETWEEN effective_start_date AND effective_END_date
        AND PL_ID IN (11871,9871,10871,7871,7873,7874,7875,247,246,4871,262,275
        ,272,258,254,256,277,260,250,278,252,264)
       AND pl_typ_id IN (86,22,26,32,87,85,31,24,37,30,25,23,29,27,28,2528,2529,508,4528,129)) BPL,
       (SELECT *
       --FROM ben.ben_opt_f --code commented by RXNETHI-ARGANO, 03/MAY/2023
	   FROM apps.ben_opt_f --code added by RXNETHI-ARGANO, 03/MAY/2023
       WHERE trunc(p_date) BETWEEN effective_start_date AND effective_END_date) opt,
       --ben.BEN_OIPL_F opl,--code commented by RXNETHI-ARGANO, 03/MAY/2023
       apps.BEN_OIPL_F opl,--code added by RXNETHI-ARGANO, 03/MAY/2023
       (SELECT papf_cont.national_identifier dep_social_num,
               ...
               papf_cont.person_id dep_person_id
                FROM PER_ALL_PEOPLE_F papf_cont,
                      ...
                      --ben.BEN_ELIG_CVRD_DPNT_F dep, --code commented by RXNETHI-ARGANO, 03/MAY/2023
                      apps.BEN_ELIG_CVRD_DPNT_F dep, --code commented by RXNETHI-ARGANO, 03/MAY/2023
    ...
    --   AND opl.oipl_id IN (783,785,1141,787,786,788,784,1275,1297,1276,1296,1281,1302,1280,1301,1279,1300,1278,1299,1277,1298,1282,1303,1283,1304,1288,1309,1287,1308,1286,1307,1285,1306,1284,1305,711,710,3515,7520,3530,3519,3518,3517,7519,3520,7517,3531,3524,3523,3522,7518,713,4522,1137,717,716,715,4521,718,4524,1138,722,721,720,4523,782,712,723,724,1139,727,726,725,800,801,1144,804,803,802,728,729,1140,732,731,730)
       ...
       AND hr_person_type_usage_info.get_user_person_type ( trunc(p_date), pap.person_id ) = 'PEO Employee' --V1.1 change
...
XXTTEC_EMP_BEN_EN_EXTR_2016.WRITE_PROCESS('TELETECH_PEO_ENROLLMENT_EXTR_2016_'||V_DT_TIME||'.txt','','W',V_PATH);
...
fnd_file.put_line(fnd_file.output,v_columns_str);
    ...
    XXTTEC_EMP_BEN_EN_EXTR_2016.write_process('TELETECH_PEO_ENROLLMENT_EXTR_2016_'||v_dt_time||'.txt', v_columns_str,'A',v_path);
...
),'||SQ||'A'||SQ||','||SQ||' BAL003'||SQ||','||SQ||' BAL004'||SQ|| '),
10871,DECODE(( '||
' SELECT DECODE((SELECT COUNT(*) '||
 ' FROM PER_PERSON_ANALYSES PAA,
     PER_ANALYSIS_CRITERIA PAC,
...
AND '||SQ||v_date||SQ||' BETWEEN PAP3.EFFECTIVE_START_DATE AND EFFECTIVE_END_DATE
AND PAA.PERSON_ID = PAP.PERSON_ID),0,'||SQ||'A'||SQ||','||SQ||'B'||SQ||') FROM DUAL
),'||SQ||'A'||SQ||','||SQ||'CHOICE001'||SQ||','||SQ||'CHOICE002'||SQ|| '),
...
AND '||SQ||v_date||SQ||' BETWEEN PAP3.EFFECTIVE_START_DATE AND EFFECTIVE_END_DATE
AND PAA.PERSON_ID = PAP.PERSON_ID),0,'||SQ||'A'||SQ||','||SQ||'B'||SQ||') FROM DUAL
),'||SQ||'A'||SQ||','||SQ||'PRIM001'||SQ||','||SQ||'PRIM002'||SQ|| '),
247,'||SQ||'METLIFE48'||SQ||',246,'||SQ||'METLIFE47'||SQ||',4871,'||SQ||'DELTA07'||SQ||',262,'||SQ||'DELTA06'||SQ||
',272,'||SQ||'METLIFE46'||SQ||',258,'||SQ||'METLIFE45'||SQ||',254,'||SQ||'METLIFE50'||SQ||',256,'||SQ||'METLIFE53'||SQ||',260,'||SQ||'METLIFE52'||SQ||',250,'||SQ||'METLIFE51'||SQ||',252,'||SQ||'METLIFE49'||SQ||',264,'||SQ||'VSP008'||SQ||
',277,'||SQ||'METLIFE53'
||SQ||',278,'||SQ||'METLIFE51'||SQ||
',7871,'||SQ||'TELE001'||SQ||
',275,'||SQ||'TELE002'||SQ||
',7873,'||SQ||'TELE003'||SQ||
',7874,'||SQ||'TELE004'||SQ||
...
||     '  decode(bpl.pl_id,247,null,246,null,272,null,254,null,opt.name) vin_OPTION,'||
    ...
    '   DECODE(bpl.pl_id, 260,pen.bnft_amt,245,pen.bnft_amt,248,pen.bnft_amt,256,pen.bnft_amt,277,pen.bnft_amt,247,(SELECT TRUNC((PROPOSED_SALARY_N * 2080),3)
                                                                                       ...
                                                                                       WHERE PPP.ASSIGNMENT_ID = PAA.ASSIGNMENT_ID
                                                                                       AND   PAA.PERSON_ID     = PAP.PERSON_ID
                                                                                       AND  '||SQ||v_date||SQ||'BETWEEN PPP.change_date AND PPP.date_to
...
FROM   hr.per_all_people_f pap,
     ...
     '  AND pl_id IN (11871,9871,10871,7871,7873,7874,7875,247,246,4871,262,275,272,258,254,256,
277,260,250,278,252,264)'||
     ...
     '  (SELECT *
       ...
       AND hr_person_type_usage_info.get_user_person_type ( '||SQ||v_date||SQ||', pap.person_id ) = '||SQ||'PEO Employee'||SQ||
...
||     '  decode(bpl.pl_id,247,null,246,null,272,null,254,null,opt.name) vin_OPTION,'||
    ...
    '   pen.enrt_cvg_strt_dt effective_date, '||
     '   (SELECT MAX(rates.rt_strt_dt) '||
      '  FROM apps.ben_prtt_rt_val rates '||
       ' WHERE rates.prtt_enrt_rslt_id = pen.prtt_enrt_rslt_id '||
     '   AND '||SQ||V_DATE||SQ||' BETWEEN RT_STRT_DT AND RT_END_DT) ded_start_date, '||
      '  DECODE(pen.pl_id,9871,DECODE(SUBSTR(opt.name,1,8),'||SQ||'Post Tax'||SQ||','||SQ||'N'||SQ||','||SQ||'Y'||SQ||'),
      10871,DECODE(SUBSTR(opt.name,1,8),'||SQ||'Post Tax'||SQ||','||SQ||'N'||SQ||','||SQ||'Y'||SQ||'),
...
END XXTTEC_EMP_BEN_EN_EXTR_2016;
/
show errors;
/