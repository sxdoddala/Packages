create or replace PACKAGE BODY      ttec_phl_sick_bal_term
AS
-- Program Name:   "PHL Monthly Sick Balance - Termed  Employees"
--
-- Description:
...
-- RXNETHI-ARGANO   03/MAY/2023 R12.2 Upgrade Remediation
-- ----------       --------    --------------------------------------------------------------------
...
-- Declare variables
      ...
      l_title           VARCHAR2 (200)
         := 'PHL Monthly Sick Balance - Termed Employees - Pay Period End Date: ';
      l_endofmonth      DATE;
...
-- get requireed info for transmission
      ...
      IS
         SELECT    "Business Group"
                ...
                "Job", "Rate", "End Of Month", "Termination Date"
           FROM (SELECT DISTINCT 'PHL' "Business Group",
                                 ...
                                 NULL "Sick Dollars", job.NAME "Job",
                            ...
                            FROM per_all_people_f papf,
                                 ...
                                 (SELECT   pca1.assignment_id,
                                           ...
                                           proportion
                                      --FROM hr.pay_cost_allocations_f pca1 --code commented by RXNETHI-ARGANO, 03/MAY/2023
									  FROM apps.pay_cost_allocations_f pca1 --code added by RXNETHI-ARGANO, 03/MAY/2023
                                     WHERE pca1.effective_end_date =
                                              (SELECT MAX
                                                         (pca2.effective_end_date
                                                         )
                                                 --FROM hr.pay_cost_allocations_f pca2 --code commented by RXNETHI-ARGANO, 03/MAY/2023
                                                 FROM apps.pay_cost_allocations_f pca2 --code added by RXNETHI-ARGANO, 03/MAY/2023
          ...
          WHERE "Sick Dollars" <> 0 OR "Sick Hours" <> 0;
   BEGIN
      ...
      l_rec :=
         'Business Group|Employee Number|Employee Full Name|Hire Date|Location Code|Location|Proportion|Location Override|Client|Department|Department Override|Assignment Number|Assignment Status|Employment Category|Payrate|Salary Basis|Sick Hours|Sick Dollars|Accrual Rate per Pay Period|Job|End Of Month|Termination Date';
