create or replace PACKAGE BODY ttec_taleo_perfo_create
/************************************************************************************
...
IS
   ...
   g_user_role1          CONSTANT VARCHAR2 (100)
                                  := 'com.taleo.orion.role.business.Manager.goal';
   g_user_role2          CONSTANT VARCHAR2 (100)
                                  := 'com.taleo.orion.role.business.Manager.review';
   ...
   trunc_stat                     VARCHAR2 (100)
	  ...
	  /*
		Commented code by MXKEERTHI-ARGANO, 07/11/2023
   ...
   e_created_by                   cust.ttec_error_handling.created_by%TYPE
                                                                      := NULL;
	   */
	  --code Added  by MXKEERTHI-ARGANO, 07/11/2023
   g_rec2_w                       apps.ttec_taleoperf_empcreate_tbl%ROWTYPE;
   -- record to write to file
...
*/
   ...
   BEGIN
     ...
     g_rec2_db.email := SUBSTR (replace(ggg.ttec_email, ' ', ''), 1, 150);
      ...
      g_rec2_db.manager_level2 := g_user_role2;
...
/*---------------------------------------------------------------------------------------------------------
   ...
   BEGIN
      ...
      e_error_message := SUBSTR (SQLERRM, 1, 240);
	  		-- cust.ttec_process_error (e_application_code, -- Commented code by MXKEERTHI-ARGANO, 07/11/2023
    apps.ttec_process_error (e_application_code,--  code Added by MXKEERTHI-ARGANO, 07/11/2023
...
*/
   ...
   BEGIN
      e_module_name := 'TALEO_C_INSERT_DB';
		--    INSERT INTO cust.ttec_taleoperf_empcreate_db  -- Commented code by MXKEERTHI-ARGANO, 07/11/2023
	    INSERT INTO apps.ttec_taleoperf_empcreate_db--  code Added by MXKEERTHI-ARGANO, 07/11/2023
...
-- Filehandle Variables
      ...
      l_email_body1     VARCHAR2 (256)
         := 'Running Concurrent Program: TeleTech Taleo Performance Employee Create File Write ';
...
/* get parent request id */
        /*
          OPEN c_parent_request_id (FND_GLOBAL.CONC_REQUEST_ID);
      FETCH c_parent_request_id
       INTO l_parent_req_id;
      CLOSE c_parent_request_id;
      */
      l_parent_req_id := FND_GLOBAL.CONC_REQUEST_ID;
      fnd_file.put_line (fnd_file.LOG,
                         '********************************** 3 '||to_char( l_parent_req_id) );
      OPEN c_directory_path(l_parent_req_id);
...
-------------------------------------------------------------------------------------------------------------------------
		...
		--             INSERT INTO cust.ttec_taleoperf_empsenttotaleo -- Commented code by MXKEERTHI-ARGANO, 07/11/2023
	             INSERT INTO apps.ttec_taleoperf_empsenttotaleo--  code Added by MXKEERTHI-ARGANO, 07/11/2023
...
---------------------------------------------------------------------------------------------------------------------------
      ...
      l_body := 'Taleo Peroformance Employee Create Record ';
      ...
      send_email (ttec_library.XX_TTEC_SMTP_SERVER, /*rehosting changes for smtp server */
	              --l_host_name,
                  l_email_from,
                  email_to_list,
                  email_cc_list,
   ...
   EXCEPTION
      ...
      THEN
         ...
         raise_application_error
              (-20003,
                  'Exception OTHERS in TeleTech Taleo Employee File Create: '
               || l_msg
              );
         ROLLBACK;
   END main;
