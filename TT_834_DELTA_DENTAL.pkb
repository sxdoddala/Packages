create or replace PACKAGE BODY      tt_834_delta_dental IS
   /* $Header: tt_834_delta_dental.pkb  ship $ */
   ...
   Call From:
    ...
    -------  --------  -------------    ------------------------------------------------------------------------------
        ...
        1.1 03/13/2014 C. Chan          Commented out copy file
        1.2 05/16/2014 K. Gonugontla    ?????
   \*== END ==================================================================================================*/
  ------------------------------------------------------------------------------
  ...
  BEGIN
    tt_log.info(p_message => p_message, p_copy_to_output => p_copy_to_output, p_additional_depth => 1);
  END info;
  ...
  BEGIN
    tt_log.warn(p_message => p_message, p_copy_to_output => p_copy_to_output, p_additional_depth => 1);
  END warn;

  ------------------------------------------------------------------------------
  -- Get HR Locations DFF
  ------------------------------------------------------------------------------
  ...
  BEGIN
    RETURN tt_log.get_descriptive_flexfield(p_application_short_name => 'PER',
                                            ...
                                            p_application_column_name => 'ATTRIBUTE4',
  ...
  BEGIN
    RETURN tt_log.get_aol_lookup_type(p_application_short_name => 'PER',
                                      ...
                                      p_lookup_type => 'TT_834_DELTA_DENTAL_EMAIL_LIST',
  ...
  (
    ...
    l_hr_locations_att4_dff        tt_log.r_descriptive_flexfield;
  ...
  BEGIN
    ...
    -- Location Code for REF*17 record
    l_hr_locations_att4_dff := get_hr_locations_att2_dff;
    ...
    OPEN tt_834_formatter.employee_cursor(p_trunc_effective_date => p_params834.trunc_effective_date,
                                          p_plan_name_like => c_plan_name_like);
    LOOP
      ...
      BEGIN
        ...
        THEN
          tt_log.assert(p_condition => l_employee.emp_full_part_flag IS NOT NULL,
          ...
          CASE l_employee.emp_full_part_flag
            ...
            WHEN 'V' THEN
              l_ins08_emp_status_code := tt_834_metadata.ins08_pt_part_time; -- 1.2
          END CASE;
        ...
        tt_log.assert(p_condition => l_employee.emp_dd_location IS NOT NULL,
                      p_neg_condition_text => l_hr_locations_att4_dff.end_user_column_name || ' IS NULL',
                      p_context => 'for Location ' || tt_log.show_value(l_employee.location_code) || ' (location_id=' ||
                                   tt_log.show_value(p_value => l_employee.location_id, p_alternate_text_when_null => NULL) || ')');

        tt_log.assert(p_condition => l_employee.opt_coverage_level_code IS NOT NULL,
                      p_neg_condition_text => l_hd05_coverage_level_code_dff.end_user_column_name || ' IS NULL',
                      p_context => 'for Benefit Option ' || tt_log.show_value(l_employee.opt_name));
	...
	LOOP
	  ...
	  EXIT WHEN tt_834_formatter.dependent_cursor%NOTFOUND;
          ...
          BEGIN
            ...
            tt_log.assert(p_condition => l_dependent.dep_ins02_individual_rel_code IS NOT NULL,
	                  p_neg_condition_text => 'Dependent ' || tt_log.show_value(l_ins02_individual_rel_dff.end_user_column_name) ||
	                                          ' IS NULL',
	                  p_context => 'for Contact Type ' || tt_log.show_value(l_dependent.dep_contact_type));

	    IF l_dependent.dep_ins02_individual_rel_code != tt_834_formatter.c_contact_dff_00_exclude
	      THEN
	        tt_log.assert(p_condition => l_dependent.dep_ins02_individual_rel_code IS NOT NULL,
	                      p_neg_condition_text => 'Dependent ' || l_ins02_individual_rel_dff.end_user_column_name || ' IS NULL',
	                      p_context => 'for Contact Type ' || tt_log.show_value(l_dependent.dep_contact_type));
	        tt_log.assert(p_condition => l_dependent.dep_first_name IS NOT NULL,
	                      p_neg_condition_text => 'Dependent First Name IS NULL');
	        tt_log.assert(p_condition => l_dependent.dep_last_name IS NOT NULL,
	                      p_neg_condition_text => 'Dependent Last Name IS NULL');
	        tt_log.assert(p_condition => l_dependent.dep_dob IS NOT NULL,
	                      p_neg_condition_text => 'Dependent Date of Birth IS NULL');
	        tt_log.assert(p_condition => l_dependent.dep_cvg_start_date IS NOT NULL,
	      ...
	      */
	      ...
	      /*
	       IF l_dependent.dep_sex IS NOT NULL
	         THEN
	           warn('Dependent SEX IS NULL for ' || tt_log.show_value(l_dependent.dep_last_name) || ', ' ||
	                tt_log.show_value(l_dependent.dep_first_name) || ' for Employee Number ' ||
	                tt_log.show_value(l_employee.emp_number) || ' (' || tt_log.show_value(l_employee.emp_last_name) || ', ' ||
	                tt_log.show_value(l_employee.emp_first_name) || ')', TRUE);
               END IF;
	      ...
	      WHEN OTHERS THEN
	        ...
	        tt_log.output_error(p_sqlcode => SQLCODE,
	                            p_sqlerrm => SQLERRM,
	                            p_message => 'for Dependent ' || tt_log.show_value(l_dependent.dep_last_name) || ', ' ||
	                                         tt_log.show_value(l_dependent.dep_first_name) || ' for Employee Number ' ||
	        ...
	        tt_log.log_contextual_exception(p_log_level => tt_log.c_log_level_error,
	                                        ...
	                                        p_sqlerrm => SQLERRM,
	                                        p_backtrace => dbms_utility.format_error_backtrace,
	                                        p_context => 'while processing ' || tt_834_formatter.describe_dependent(l_dependent));
	      END;
	...
	END LOOP;
        ...
        l_insured.loop2000_2nd_ref_qualifier  := tt_834_metadata.ref01_1l_group_number;
        l_insured.loop2000_2nd_ref_identifier := l_employee.grp_number;--c_ref_0000109_group_number;-- Fix for TASK0031904 on 2/25
        l_insured.loop2000_3rd_ref_qualifier  := tt_834_metadata.ref01_17_client_reporting_cat;
        l_insured.loop2000_3rd_ref_identifier := l_employee.emp_dd_location;
        l_insured.loop2000_4th_ref_qualifier  := tt_834_metadata.ref01_dx_depart_agency_number;
        l_insured.loop2000_4th_ref_identifier := c_ref_ddpusa_depart_agency_num;
        ...
        tt_834_formatter.write_insured_record(p_params834 => p_params834, p_insured => l_insured);
      ...
      EXCEPTION
        WHEN OTHERS THEN
          ...
          tt_log.output_error(p_sqlcode => SQLCODE,
                              p_sqlerrm => SQLERRM,
                              p_message => 'for Employee Number ' || tt_log.show_value(l_employee.emp_number) || ' (' ||
                                           tt_log.show_value(l_employee.emp_last_name) || ', ' ||
                                           tt_log.show_value(l_employee.emp_first_name) || ')');
          tt_log.log_contextual_exception(p_log_level => tt_log.c_log_level_error,
                                          p_sqlcode => SQLCODE,
                                          p_sqlerrm => SQLERRM,
                                          p_backtrace => dbms_utility.format_error_backtrace,
                                          p_context => 'while processing ' || tt_834_formatter.describe_employee(l_employee));
      END;
    END LOOP;
    ...
    tt_file_metadata.file_close(p_file_handle => p_params834.file_handle, p_ignore_exceptions => FALSE);

    ------------------------------------------------------------------------------
    -- Summarize results
    ------------------------------------------------------------------------------
    tt_log.output(p_message => tt_log.show_value(l_employee_success_count) || ' employee record(s) processed successfully.');
    tt_log.output(p_message => tt_log.show_value(l_dependent_success_count) || ' dependent record(s) processed successfully.');

    -- Raise exception if any insured records caused an error
    tt_log.assert(p_condition => l_employee_error_count = 0,
                  p_neg_condition_text => tt_log.show_value(l_employee_error_count) || ' employee record(s) had errors');
    tt_log.assert(p_condition => l_dependent_error_count = 0,
                  p_neg_condition_text => tt_log.show_value(l_dependent_error_count) || ' dependent record(s) had errors');
  EXCEPTION
    WHEN OTHERS THEN
      ...
      tt_file_metadata.file_close(p_file_handle => p_params834.file_handle, p_ignore_exceptions => TRUE);
  ...
  BEGIN
    ...
    tt_log.output(p_message => c_program_name || chr(10) || rpad('=', length(c_program_name), '='));
    ...
    -- customize metadata
    tt_834_metadata.isa06_def.charset := c_digits_only;
    tt_834_metadata.isa08_def.charset := c_digits_only;
    tt_834_metadata.gs02_def.charset := c_digits_only;
    tt_834_metadata.gs03_def.charset := c_digits_only;
    tt_834_metadata.n104_def.charset := c_digits_only;
    ...
    tt_834_metadata.ref02_def_hash(tt_834_metadata.ref01_1l_group_number).charset := c_digits_only;
    tt_834_metadata.ref02_def_hash(tt_834_metadata.ref01_17_client_reporting_cat).charset := c_digits_only;
    ...
    info('  p_test_prod                = ' || tt_log.show_value(p_test_prod), TRUE);
    info('  p_effective_date           = ' || tt_log.show_value(p_effective_date), TRUE);
    info('  p_initial_output_directory = ' || tt_log.show_value(p_initial_output_directory), TRUE);
    info('  p_initial_output_filename  = ' || tt_log.show_value(p_initial_output_filename), TRUE);
    info('  p_copy_to_directory        = ' || tt_log.show_value(p_copy_to_directory), TRUE);
    info('  p_copy_to_filename         = ' || tt_log.show_value(p_copy_to_filename), TRUE);
    info('  p_log_level_code           = ' || tt_log.show_value(p_log_level_code), TRUE);
    ...
    -- Check P_TEST_PROD
    tt_log.assert(p_condition => p_test_prod = c_mode_test OR p_test_prod = c_mode_prod,
                  p_neg_condition_text => 'parameter p_test_prod is not one of: ' || tt_log.show_value(c_mode_test) || ', ' ||
                                          tt_log.show_value(c_mode_prod),
                  p_context => 'because p_test_prod = ' || tt_log.show_value(p_test_prod));
  /* 1.1
    -- Check/Parse P_EFFECTIVE_DATE
    tt_log.assert(p_condition => p_effective_date IS NOT NULL, p_neg_condition_text => 'parameter p_effective_date IS NULL');
  ...
  */
    ...
    tt_log.assert(p_condition => p_initial_output_directory IS NOT NULL,
                  p_neg_condition_text => 'parameter p_initial_output_directory IS NULL');
    tt_log.assert(p_condition => p_initial_output_filename IS NOT NULL,
                  p_neg_condition_text => 'parameter p_initial_output_filename IS NULL');
    tt_log.assert(p_condition => p_copy_to_directory IS NOT NULL, p_neg_condition_text => 'parameter p_copy_to_directory IS NULL');
    tt_log.assert(p_condition => p_copy_to_filename IS NOT NULL, p_neg_condition_text => 'parameter p_copy_to_filename IS NULL');
    l_initial_output_filename := tt_log.replace_date_format_mask(p_filename => p_initial_output_filename, p_date => l_extract_time);
    l_copy_to_filename        := tt_log.replace_date_format_mask(p_filename => p_copy_to_filename, p_date => l_extract_time);

    -- Check P_LOG_LEVEL_CODE
    tt_log.assert(p_condition => p_log_level_code IS NOT NULL, p_neg_condition_text => 'parameter p_log_level_code IS NULL');

    ------------------------------------------------------------------------------
    -- Display parsed paramters
    ------------------------------------------------------------------------------
    info(chr(10) || 'Effective Parameters:', TRUE);
    info('  Extract Time / Current Time = ' || tt_log.show_value(l_extract_time), TRUE);
    info('  Effectitve Date for Extract = ' || tt_log.show_value(l_trunc_effective_date), TRUE);
    info('  Initial Output Directory    = ' || tt_log.show_value(tt_log.filename_logical_to_physical(p_initial_output_directory)),
         TRUE);
    info('  Initial Output Filename     = ' || tt_log.show_value(l_initial_output_filename), TRUE);
    info('  Copy To Directory           = ' || tt_log.show_value(tt_log.filename_logical_to_physical(p_copy_to_directory)), TRUE);
    info('  Copy To Filename            = ' || tt_log.show_value(l_copy_to_filename), TRUE);
    info('  Security Group              = ' || tt_log.show_value(tt_log.get_security_group_name(fnd_global.security_group_id)),
         TRUE);

    tt_log.assert(p_condition => l_extract_time IS NOT NULL, p_neg_condition_text => 'l_extract_time IS NULL');
    tt_log.assert(p_condition => l_trunc_effective_date IS NOT NULL, p_neg_condition_text => 'l_effective_date IS NULL');
    ...
    l_params834.ref02_master_policy_number   := NULL;
    ...
    ------------------------------------------------------------------------------
    -- Acutal File Generation
    ------------------------------------------------------------------------------
    l_task := tt_log.start_task(p_log_level => tt_log.c_log_level_info, p_task_name => 'File Generation', p_copy_to_output => TRUE);
    ...
    tt_log.stop_task(p_sqlcode => SQLCODE, p_sqlerrm => SQLERRM, p_task => l_task);
...
*/
  ...
  EXCEPTION
    WHEN OTHERS THEN
      ...
      tt_log.output_error(p_sqlcode => SQLCODE, p_sqlerrm => SQLERRM, p_message => 'during file generation');
