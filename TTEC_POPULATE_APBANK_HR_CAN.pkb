create or replace PACKAGE BODY ttec_populate_apbank_hr_CAN
AS
--************************************************************************************--
--*                                                                                  *--
--*     Program Name: ttec_populate_apbank_hr_CAN                                    *--
--*                                                                                  *--
...
--*---------             ----        -----------                                     *--
--*                                                                                  *--
--*   Wasim Manasfi   04/13/2007  Created                                             *--
--* Changed by rajnish for R12 retrofit                                              *--
...
-- Filehandle Variables
   ...
   IS
      --l_error_message               cust.ttec_error_handling.error_message%TYPE;   --code commented by RXNETHI-ARGANO,17/05/23
      l_error_message               apps.ttec_error_handling.error_message%TYPE;     --code added by RXNETHI-ARGANO,17/05/23
   ...
   BEGIN
      ...
      LOOP
         ...
         v_bank_name :=    lvendor.segment5; -- Changed to be unique to employee lvendor.segment1;
         v_account_type := lvendor.segment2;
         v_account_number := lvendor.segment3;
         v_routing_number := lpad(lvendor.segment7||lvendor.segment4, 9, '0');  -- <-- only difference in CAN version
         v_bank_name_5 := nvl(lvendor.segment5, substr(v_account_number, -5, 5)||'315');
         ...
         BEGIN
            SELECT DISTINCT branch_party_id
                      ...
                      WHERE bank_number = v_routing_number
 ...
 l_ext_bank_rec.country_code :='CA';
...
,x_response => l_response_rec
            ...
            END IF;
               ...
               l_ext_bank_branch_rec.branch_number := v_routing_number;
...
-- need to commit this to move on to create the bank account, if no branch then no account
               BEGIN
                  ...
                  l_ext_bank_act_rec.country_code := 'CA';
                  ...
                  l_ext_bank_act_rec.currency := 'CAD';
...
END ttec_populate_apbank_hr_CAN;
/
show errors;
/