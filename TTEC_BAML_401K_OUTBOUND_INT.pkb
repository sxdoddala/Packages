create or replace PACKAGE BODY      TTEC_BAML_401K_OUTBOUND_INT AS
--
-- Program Name:  TTEC_BAML_401K_OUTBOUND_INT
-- /* $Header: TTEC_BAML_401K_OUTBOUND_INT.pkb 1.0 2011/10/14  chchan ship $ */
--
-- /*== START ================================================================================================*\
    ...
    *********************************************************/
    ...
    BEGIN
                ...
                v_rec :=  'Field 1    RECORD TYPE'
                   ...
                   ||'|'||'Field 26   SOURCE 3 LABEL- Blank'
                   ||'|'||'Field 27   SOURCE 3 AMOUNT - Send 0'
                   ||'|'||'Field 28   SOURCE 4 LABEL- Blank'
                   ||'|'||'Field 29   SOURCE 4 AMOUNT - Send 0'
                   ||'|'||'Field 30   SOURCE 5 LABEL- Send Blank'
                   ||'|'||'Field 31   SOURCE 5 AMOUNT - Send 0'
                   ||'|'||'Field 32   SOURCE 6 LABEL- Send Blank'
                   ||'|'||'Field 33   SOURCE 6 AMOUNT - Send 0'
                   ||'|'||'Field 34   PAY PERIOD COMPENSATION - Send 0'
    ...
    PROCEDURE get_emp_address(p_person_id IN NUMBER,
                             ...
                             p_country_code OUT VARCHAR2) IS
    ...
    BEGIN
      ...
      SELECT --pad.address_line1, pad.address_line2, /* 5.3 */
        ...
        --FROM hr.per_addresses pad		-- Commented code by IXPRAVEEN-ARGANO,09-May-2023
		FROM apps.per_addresses pad		--  code Added by IXPRAVEEN-ARGANO,09-May-2023
        WHERE pad.person_id = p_person_id
        AND pad.primary_flag = 'Y'
        AND p_effective_start_date BETWEEN pad.date_from AND NVL(pad.date_to,'31-DEC-4712');

      /* 7.4 Begin */
      p_address_line1:= TRIM(REGEXP_REPLACE(p_address_line1,'[^' || CHR(1) || '-' || CHR(127) || ']',''));
      p_address_line2:= TRIM(REGEXP_REPLACE(p_address_line2,'[^' || CHR(1) || '-' || CHR(127) || ']',''));
      p_town_or_city:= TRIM(REGEXP_REPLACE(p_town_or_city,'[^' || CHR(1) || '-' || CHR(127) || ']',''));
      p_region_2:= TRIM(REGEXP_REPLACE(p_region_2,'[^' || CHR(1) || '-' || CHR(127) || ']',''));
      p_postal_code:= TRIM(REGEXP_REPLACE(p_postal_code,'[^' || CHR(1) || '-' || CHR(127) || ']',''));
      p_country_code:= TRIM(REGEXP_REPLACE(p_country_code,'[^' || CHR(1) || '-' || CHR(127) || ']',''));
      /* 7.4 End  */
...
--      apps.Fnd_File.put_line (apps.Fnd_File.log,'p_actual_termination_date ->'||p_actual_termination_date);
     ...
     IF p_actual_termination_date IS NOT NULL THEN
        ...
        INTO p_salary
        --FROM   hr.per_pay_proposals ppp				-- Commented code by IXPRAVEEN-ARGANO,09-May-2023
		FROM   apps.per_pay_proposals ppp				--  code Added by IXPRAVEEN-ARGANO,09-May-2023
            ,  per_pay_bases ppb
        ...
        AND ppp.change_date     = (SELECT MAX(ppp1.change_date)
                                   --FROM hr.per_pay_proposals ppp1			-- Commented code by IXPRAVEEN-ARGANO,09-May-2023
								   FROM apps.per_pay_proposals ppp1			--  code Added by IXPRAVEEN-ARGANO,09-May-2023
    ...
    BEGIN
       ...
       SELECT balance_type_id
         ...
         --FROM hr.pay_balance_types	         -- Commented code by IXPRAVEEN-ARGANO,09-May-2023
		 FROM apps.pay_balance_types			--  code Added by IXPRAVEEN-ARGANO,09-May-2023
       ...
       SELECT balance_dimension_id
        ...
        -- FROM hr.pay_balance_dimensions				-- Commented code by IXPRAVEEN-ARGANO,09-May-2023
		 FROM apps.pay_balance_dimensions				--  code Added by IXPRAVEEN-ARGANO,09-May-2023
       ...
       SELECT defined_balance_id
         ...
         --FROM hr.pay_defined_balances				-- Commented code by IXPRAVEEN-ARGANO,09-May-2023
		 FROM apps.pay_defined_balances					--  code Added by IXPRAVEEN-ARGANO,09-May-2023
    ...
    BEGIN
        ...
        SELECT pdb.defined_balance_id
          INTO l_defined_balance_id
         -- FROM hr.pay_defined_balances pdb				-- Commented code by IXPRAVEEN-ARGANO,09-May-2023
		  FROM apps.pay_defined_balances pdb				--  code Added by IXPRAVEEN-ARGANO,09-May-2023
        ...
        SELECT  pbt.balance_name
          INTO l_balance_name
         -- FROM hr.pay_balance_types pbt					-- Commented code by IXPRAVEEN-ARGANO,09-May-2023
		  FROM apps.pay_balance_types pbt					--  code Added by IXPRAVEEN-ARGANO,09-May-2023
...
--                                                      ||' Effective Date: '|| p_effective_date);
         ...
         THEN
             SELECT SUM(NVL(prb.balance_value,0))
               INTO l_value
              --FROM hr.pay_run_balances prb			-- Commented code by IXPRAVEEN-ARGANO,09-May-2023
			  FROM apps.pay_run_balances prb			--  code Added by IXPRAVEEN-ARGANO,09-May-2023
...
--START R12.2 Upgrade Remediation
 /* v_emp_addr_line1        hr.per_addresses.address_line1%TYPE;				-- Commented code by IXPRAVEEN-ARGANO,09-May-2023
	...
	v_emp_addr_line1        apps.per_addresses.address_line1%TYPE;				--  code Added by IXPRAVEEN-ARGANO,09-May-2023
    ...
    BEGIN
       ...
       IF p_balance_name = p_401k_bal_name THEN
            ...
            SELECT f23_source1_401kpretax
              INTO l_value
              --FROM cust.ttec_peo_401k_load			-- Commented code by IXPRAVEEN-ARGANO,09-May-2023
			  FROM apps.ttec_peo_401k_load				--  code Added by IXPRAVEEN-ARGANO,09-May-2023
...
--           send_PEO_email(p_email_recipients);
    ...
    BEGIN
        ...
        SELECT pbd.balance_dimension_id
          ...
          --FROM hr.pay_balance_dimensions pbd			-- Commented code by IXPRAVEEN-ARGANO,09-May-2023
		  FROM apps.pay_balance_dimensions pbd			--  code Added by IXPRAVEEN-ARGANO,09-May-2023
    ...
    v_rec :=  'UHDR'                                                                      -- Field 1    CONSTANT -Value of "UHDR" (constant)
           ...
           ||'|'|| SUBSTR (LPAD('610115',6,'0'),1,6)                                      -- Field 4    ML PLAN NUMBER - Six-digit plan number (assigned by Merrill Lynch)
           ...
           ||'|'|| TO_CHAR( g_check_date,'MMDDRRRR')                              -- Field 15   PAYCHECK DATE (JULIAN)- The payroll effective start date
...
--                            v_emp_salary) ;
           ...
           /* V1.5 End*/
            ...
            v_rec :=  '71'                                                                        -- Field 1    RECORD TYPE-Populate 71
                   ||'|'|| SUBSTR (LPAD('610115',6,'0'),1,6)                                      -- Field 2    FILLER - Space filled or BLANK
                   ...
                   ||'|'|| RPAD(' ',15,' ')                                                       -- Field 7    LAST NAME - Send Blank
                   ||'|'|| RPAD(' ',15,' ')                                                       -- Field 8    FIRST  NAME - Send Blank
                   ||'|'|| SUBSTR (RPAD( UPPER(emp_rec.full_name),30,' '),1,30)                   -- Field 9    FULL NAME - PER_ALL_PEOPLE_F.FULL_NAME
                   ...
                   ||'|'|| RPAD(' ',8,' ')                                                        -- Field 12   DATE FIRST ELIGIBLE -  Not Required. Send blank.
...
--                           ))                                                                     -- Field 14   ALTERNATE VEST DATE -  Not Required. Send Blank
                   ...
                   ||'|'|| 'B'                                                                    -- Field 15   PAYROLL FREQUENCY - Required. Indicates frequency of participant's paycheck; W=weekly, B= bi-weekly, S=semi-monthly, M=monthly; will be used to amortize loans
                   ...
                   ||'|'|| 'Z'                                                                    -- Field 24   SOURCE 2 LABEL- Z Z for Catchup; if catchup is reported separately
...
--                   ||'|'|| NVL(SUBSTR (LPAD(REPLACE(REPLACE(REPLACE(TO_CHAR(v_401K_ER_amount,'000000.99S'),'.',''),'+','{'),'-','{'),9,'0'),1,9),RPAD(' ',9,' '))                               -- Field 27   SOURCE 3 AMOUNT - Required. Element Name is Pre Tax 401K ER
                   ||'|'|| ' '                                                                    -- Field 26   SOURCE 3 LABEL- X for ER Match
                   ||'|'|| '000000000'                                                            -- Field 27   SOURCE 3 AMOUNT - Required. Element Name is Pre Tax 401K ER
                   ||'|'|| ' '                                                                    -- Field 28   SOURCE 4 LABEL- Send Blank
                   ||'|'|| '000000000'                                                            -- Field 29   SOURCE 4 AMOUNT - Send 0's
                   ||'|'|| ' '                                                                    -- Field 30   SOURCE 5 LABEL- Send Blank
                   ||'|'|| '000000000'                                                            -- Field 31   SOURCE 5 AMOUNT - Send 0's
                   ||'|'|| ' '                                                                    -- Field 32   SOURCE 6 LABEL- Send Blank
                   ||'|'|| '000000000'                                                            -- Field 33   SOURCE 6 AMOUNT - Send 0's
                   ||'|'|| '000000000'                                                            -- Field 34   PAY PERIOD COMPENSATION - Send 0's
...
--                   ||'|'|| v_officer_flag                                                       -- Field 61   OKEY EMPLOYEE - Send 1 for emp no 3012468 and send 0 for rest all employees.
                  ...
                  -- ||'|'|| RPAD('N',1,' ')                                                        -- Field 65   ELIGIBILITY FLAG - Populate N /* 7.6 */
                   ...
                   ||'|'|| NVL(SUBSTR (LPAD(UPPER( emp_rec.email_address),50,' '),1,50),RPAD(' ',50,' '))                       -- Field 79   BUSINESS E-MAIL ADDRESS - Per_all_people_f.email_address
...
--                   || v_officer_flag                                                       -- Field 61   OKEY EMPLOYEE - Send 1 for emp no 3012468 and send 0 for rest all employees.
    ...
    v_rec :=  'UTRL'                                                                      -- Field 1    RECORD TYPE-Populate UTRL
           ...
           ||'|'|| SUBSTR (LPAD('610115',6,'0'),1,6)                                      -- Field 4    ML PLAN NUMBER - Six-digit plan number (assigned by Merrill Lynch) -Populate 610115
           ...
           ||'|'|| RPAD('Z',3,' ')                                                        -- Field 9    SOURCE 2 LABEL- Z Z for Catchup; if catchup is reported separately
    ...
    THEN
         ...
         ttec_error_logging.process_error( g_application_code -- 'BEN'
                                         ...
                                         , g_package          -- 'TTEC_BAML_401K_OUTBOUND_INT'
                                         , v_module
                                         , g_failure_status
                                         , SQLCODE
                                         , SQLERRM
    ...
    THEN
        ...
        RAISE_APPLICATION_ERROR(-20003,'Exception OTHERS in TTEC_BAML_401K_OUTBOUND_INT.main: '||'Module >-' ||v_module||' ['||g_label1||']['||v_loc||']['||g_label2||']['||g_emp_no|| '] ERROR:'||errbuff);
    END main;
